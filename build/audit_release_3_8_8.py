...
try:
    ...
    if meta.get('localgptVersion')!='3.8.8': errors.append('docs/docfx.json localgptVersion != 3.8.8')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.8.8**'),('docs/pdf/toc.yml','LocalGPT-3.8.8.pdf'),
    ('RELEASE.md','# LocalGPT 3.8.8'),('CHANGELOG-v3.8.8-SEED-MANIFEST-BUILD-CONTRACT-REPAIR.md','seed manifest/build contract repair'),
    ('VALIDATION-v3.8.8-source.md','# LocalGPT 3.8.8 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.8.8'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.8.8')): req(rel,mark)
...
# 3.8.8 application-owned storage contract. Mutable LocalGPT state remains per-user by default;
...
# 3.8.8 boot dependency-cycle repair: preserve ordinary hosted services while preventing
...
# 3.8.8 clean-source seed contract
...
if not _seed.is_file():
    ...
    if "does not execute this SQL file automatically" not in _seed_text:
        errors.append("Council seed manifest must state non-execution ownership")
_prereq = read("build/Initialize-BuildPrerequisites.ps1")
if "docs/COUNCIL_KNOWLEDGE_SEED.sql" not in _prereq:
    errors.append("clean-source preflight must validate the packaged Council seed manifest")

if errors:
    print('LocalGPT 3.8.8 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.8.8 source audit passed.')
