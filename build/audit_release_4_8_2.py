#!/usr/bin/env python3
"""Source-only release audit for LocalGPT 4.8.2. Never invokes dotnet, GitHub, installers, or project code."""
...
def main() -> int:
    ...
    require("src/LocalGPT/Components/App.razor", "localgpt-game-console.js?v=4.8.2", "localgpt-chat-ui.js?v=4.8.2")
    require("docs/index.md", "**Version 4.8.2**", "LocalGPT-4.8.2.pdf")
    require("docs/docfx.json", '"localgptVersion": "4.8.2"')
    require("docs/pdf/toc.yml", "LocalGPT-4.8.2.pdf")
    require("docs/pdf-cover.html", "LocalGPT 4.8.2 Documentation", "Version 4.8.2")
    require("CHANGELOG-v4.8.2-DOCUMENTATION-BROWSER-CHUNK-RECOVERY.md", "# LocalGPT 4.8.2")
    require("VALIDATION-v4.8.2-source.md", "# LocalGPT 4.8.2 source validation")
    ...
    # 4.8.1 maintenance-policy repair remains intact: fix code, do not weaken the guards.
    ...
    # 4.8.2 documentation browser/chunk recovery.
    require(
        "src/LocalGPT/LocalGPT.csproj",
        "LocalGPT.ReleasePackaging\\LocalGPT.ReleasePackaging.csproj",
        'ReferenceOutputAssembly="false"',
    )
    require(
        "Directory.Build.targets",
        "LocalGptDocumentationPackagingToolRoot",
        "LocalGPT.ReleasePackaging.exe",
        '-PackagingTool &quot;$(LocalGptDocumentationPackagingTool)&quot;',
    )
    require(
        ...
        '"brave-browser"',
        "Documentation browser resolved:",
        "$requiresChunkedBrowserPdf = $pdfSourcePageCount -gt $browserPdfChunkPages",
        "will not misreport this as a missing browser",
        "The bounded browser PDF path did not produce a complete document",
    )
    forbid(
        "build/Build-Documentation.ps1",
        "$browserPdfChunkPages = 100\n",
        "$browserPdfChunkPages = 120\n",
        "Browser printing was unavailable; generating the complete PDF",
    )
    require(
        "build/Build-Documentation.ps1",
        "The monolithic DocFX/Playwright fallback is disabled for chunked or low-memory documentation builds",
        "FUTURE2_DOCUMENTATION_ALLOW_MONOLITHIC_PDF_FALLBACK=1",
    )
    ...
    print("LocalGPT 4.8.2 source audit: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"LocalGPT 4.8.2 source audit: FAIL: {exc}", file=sys.stderr)
        raise
