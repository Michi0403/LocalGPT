#!/usr/bin/env python3
"""Static source audit for LocalGPT 3.1.8 Chat quick-preset isolation repair."""
...
try:
    for rel in (
        ...
        require(rel, "<Version>3.1.8</Version>")
    require("src/LocalGPT.WireProtocolVersion/LocalGPT.WireProtocolVersion.csproj", "<Version>2.1.1</Version>")
    require("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/3.1.8")
    ...
    require("build/audit_chat_quick_configuration_3_1_8.py",
            "protected pre-quick Chat CSS changed",
            "Chat Configuration markup changed",
            "DxAIChat subtree changed",
            "Renderer-affinity contract")
    require("CHANGELOG-v3.1.8-CHAT-QUICK-PRESET-ISOLATION.md",
            "3.1.5", "composer", "renderer-affine", "sibling")
    require("VALIDATION-v3.1.8-source.md",
            "3.1.8", "No `dotnet`", "3bc9693f026e410de1cd03c24544ab5695f58a13d238bc9710498eab6e090ad1")
    require("RELEASE.md", "# LocalGPT 3.1.8", "Quick preset isolation")

    subprocess.run([sys.executable, str(root / "build/audit_chat_quick_configuration_3_1_8.py")], check=True, cwd=root)
    checks += 1

    print(f"LocalGPT 3.1.8 Chat quick-preset isolation source audit passed: {checks} checks.")
except (AssertionError, ValueError, subprocess.CalledProcessError) as exc:
    print(f"LocalGPT 3.1.8 source audit failed: {exc}", file=sys.stderr)
    raise SystemExit(1)
