#!/usr/bin/env python3
"""Static integration gate for LocalGPT 4.2.4 ASCII chat, DXFunction, seed, game and controller wiring."""
...
def main() -> int:
    ...
    console = read("src/LocalGPT/Components/Shared/ChatGameConsole.razor.cs")
    ...
    for token in (
        ...
        if token not in console:
            failures.append(f"ASCII transcript/sequence contract missing {token!r}")
    ...
    if failures:
        print("LocalGPT 4.2.4 ASCII experience audit FAILED:")
        for failure in failures:
            print(f" - {failure}")
        return 1
    print("LocalGPT 4.2.4 ASCII experience audit passed: canonical replay, Council lanes, DXFunction discovery/system seeding, shipped game capabilities, bounded animations, and demand-driven controller support are intact.")
    return 0
