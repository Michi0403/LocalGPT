...
def main() -> int:
    ...
    require("CHANGELOG-v4.7.8-CONSOLE-IDENTITY-STRUCTURE-COMPILE-REPAIR.md", f"# LocalGPT {VERSION}")
    require("VALIDATION-v4.7.8-source.md", f"# LocalGPT {VERSION} source validation")
    require("src/LocalGPT/Components/App.razor", "localgpt-game-console.js?v=4.7.8", "localgpt-chat-ui.js?v=4.7.8")
    require("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.7.8")
    require("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.7.8")
    require("docs/index.md", "**Version 4.7.8**", "LocalGPT-4.7.8.pdf")
    require("docs/docfx.json", '"localgptVersion": "4.7.8"')
    require("docs/pdf/toc.yml", "LocalGPT-4.7.8.pdf")
    require("docs/pdf-cover.html", "LocalGPT 4.7.8 Documentation", "Version 4.7.8")
    ...
    print(f"LocalGPT {VERSION} release audit passed: the Local AI runtime remains present, the owner-reported 4.7.6/4.7.7 build-policy and compiler regressions are source-repaired, required media-input validation is preserved, and prior benchmark, Council, ASCII and InteractiveServer contracts remain present.")
    return 0
