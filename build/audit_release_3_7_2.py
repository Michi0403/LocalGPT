...
try:
    ...
    if data.get('build',{}).get('globalMetadata',{}).get('localgptVersion')!='3.7.2':
        errors.append('docs/docfx.json localgptVersion is not 3.7.2')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','LocalGPT-3.7.2.pdf')
req('docs/index.md','**Version 3.7.2**')
req('RELEASE.md','# LocalGPT 3.7.2')
req('CHANGELOG-v3.7.2-MACOS-PDF-TOOL-PROVISIONING.md','Version advanced from 3.7.1 to 3.7.2')
req('VALIDATION-v3.7.2-source.md','# LocalGPT 3.7.2 source validation')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.7.2')
req('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.7.2')
...
# 3.7.2/3.3.1 release-size and resume invariants.
build=text('Build-Release.ps1')
for marker in (
...
if errors:
    print('LocalGPT 3.7.2 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('LocalGPT 3.7.2 source audit passed.')
