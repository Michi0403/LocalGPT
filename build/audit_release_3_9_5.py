...
try:
    ...
    if meta.get('localgptVersion')!='3.9.5': errors.append('docs/docfx.json localgptVersion != 3.9.5')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.9.5**'),('docs/pdf/toc.yml','LocalGPT-3.9.5.pdf'),
    ('RELEASE.md','# LocalGPT 3.9.5'),('CHANGELOG-v3.9.5-DEBUG-DOCUMENTATION-PDF-CONTRACT-REPAIR.md','Debug documentation PDF contract repair'),
    ('VALIDATION-v3.9.5-source.md','# LocalGPT 3.9.5 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.9.5'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.9.5')): req(rel,mark)
...
# 3.9.4 Windows PowerShell 5.1 documentation-cache regression repair retained in 3.9.5.
...
# 3.9.5 Debug/Release documentation PDF contract repair.
...
for marker_text in (
    ...
    if marker_text not in docs_395:
        errors.append(f'3.9.5 documentation PDF contract marker missing: {marker_text}')
for marker_text in (
    ...
    if marker_text not in targets_395:
        errors.append(f'3.9.5 Directory.Build.targets PDF contract marker missing: {marker_text}')
if '-RequirePdf `' not in read('Build-Release.ps1'):
    errors.append('3.9.5 release orchestration no longer explicitly requires the complete documentation PDF')
...
if re.search(r'(?m)^if \(Test-Path -LiteralPath \$sourceWebRoot -PathType Container\) \{\s*\n\s*\$sourcePdfPath', docs_395):
    errors.append('3.9.5 still contains an unconditional source-tree PDF assertion')
...
# 3.9.5 build-guard repair. Reproduce the maintained PowerShell guard predicates in
...
for marker_text in ('bool IsOllamaProfile(AiProviderBootstrapProfile profile);',):
    if marker_text not in interface_text:
        errors.append(f'3.9.5 provider classification ownership marker missing: {marker_text}')
...
for marker_text in ('Providers.IsOllamaProfile(profile)', 'FirstOrDefault(Providers.IsOllamaProfile)'):
    if marker_text not in panel_391:
        errors.append(f'3.9.5 Setup provider-classification delegation missing: {marker_text}')
if '.StartsWith("ollama-", StringComparison.OrdinalIgnoreCase)' in panel_391:
    errors.append('3.9.5 Setup component still owns an Ollama profile-prefix StartsWith check')
...
for marker_text in ('private const string JsonMediaType = "application/json";', 'Encoding.UTF8, JsonMediaType'):
    if marker_text not in canirun_391:
        errors.append(f'3.9.5 CanIRun constructor-literal guard repair missing: {marker_text}')

# 3.9.5 packaged-runtime setup repair: the launcher, logger, optional hardware probes,
...
for marker_text in ('ResolveProcessWorkingDirectory()', 'RepairInvalidCurrentDirectory()', 'Directory.SetCurrentDirectory(ResolveProcessWorkingDirectory())'):
    if marker_text not in paths:
        errors.append(f'3.9.5 process working-directory repair missing: {marker_text}')
program_392 = read('src/LocalGPT/Program.cs')
if 'LocalGptApplicationDataPaths.RepairInvalidCurrentDirectory()' not in program_392:
    errors.append('3.9.5 startup no longer repairs an invalid inherited current directory')
...
for marker_text in ('cd "$USER_DATA_DIR/runtime"', 'durable per-user runtime working directory'):
    if marker_text not in launcher_392:
        errors.append(f'3.9.5 macOS launcher working-directory repair missing: {marker_text}')
if 'cd "$APP" || { show_failure "The packaged application directory could not be opened:' in launcher_392:
    errors.append('3.9.5 macOS launcher still makes the replaceable application bundle its process working directory')
...
for marker_text in ('ResolveLogPath(_options)', 'LocalGptApplicationDataPaths.ResolveUserPath("logs", "LocalGPT.log")', 'Path.GetTempPath()', 'AppContext.BaseDirectory'):
    if marker_text not in file_logger_392:
        errors.append(f'3.9.5 file-logger runtime-path repair missing: {marker_text}')
if 'Directory.GetCurrentDirectory()' in file_logger_392:
    errors.append('3.9.5 FileLogger still depends on Directory.GetCurrentDirectory()')
...
if 'return LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory();' not in console_392:
    errors.append('3.9.5 bounded console still lacks the stable default working directory used by provider/model installs')
if 'return Environment.CurrentDirectory;' in console_392:
    errors.append('3.9.5 bounded console still uses Environment.CurrentDirectory as its blank-request default')
...
for marker_text in ('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()', 'Optional NVIDIA discovery was unavailable', 'platformGpus = []'):
    if marker_text not in hardware_392:
        errors.append(f'3.9.5 hardware inventory best-effort repair missing: {marker_text}')
...
if platform_392.count('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()') != 2:
    errors.append('3.9.5 platform hardware probes must both use the stable process working directory')
...
for marker_text in ('Optional hardware discovery was unavailable while building initial setup snapshot', 'provider, Ollama, model, and recommendation controls remain usable.', 'hardware = [];'):
    if marker_text not in setup_392:
        errors.append(f'3.9.5 setup optional-hardware isolation missing: {marker_text}')
...
for marker_text in ('Start Ollama', 'Stop Ollama', 'Restart Ollama', 'Fetch attributed recommendations', 'Download / install model', 'Install model', 'Register endpoint in LocalGPT'):
    if marker_text not in panel_392:
        errors.append(f'3.9.5 unified setup workflow regressed required UI marker: {marker_text}')
for forbidden in ('recommendations.Take(24)', 'modelChoices.Take(32)'):
    if forbidden in panel_392:
        errors.append(f'3.9.5 unified setup workflow still hides fetched recommendations behind a Razor-only truncation: {forbidden}')
if '.Take(96)' in setup_392:
    errors.append('3.9.5 model-choice mapping still truncates CanIRun recommendations before the unified Setup form can offer each one')
canirun_392 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
if 'limit * 4' in canirun_392:
    errors.append('3.9.5 CanIRun parser retains the overflowing recommendation-limit multiplication that can collapse an unlimited policy to one result')
if 'result.Count >= 512' not in canirun_392:
    errors.append('3.9.5 CanIRun JSON traversal lost its explicit 512-object safety bound')


# 3.9.5 Windows/Mac runtime evidence repairs.
...
for marker_text in (
    ...
    if marker_text not in canirun_393:
        errors.append(f'3.9.5 CanIRun repair missing: {marker_text}')
if 'new("https://canirun.ai/api/recommend"' in canirun_393:
    errors.append('3.9.5 still starts CanIRun lookups at the redirecting non-www endpoint')
if 'AllowAutoRedirect = false' not in read('src/LocalGPT/Program.ServiceRegistration.cs'):
    errors.append('3.9.5 CanIRun HTTP client lost redirect-disabled transport policy')
...
for marker_text in (
    ...
    if marker_text not in setup_393:
        errors.append(f'3.9.5 setup isolation/model mapping repair missing: {marker_text}')
...
for marker_text in (
    ...
    if marker_text not in panel_393:
        errors.append(f'3.9.5 setup recovery/recommendation UI missing: {marker_text}')
if 'if (snapshot is null)' not in panel_393:
    errors.append('3.9.5 Setup loading contract unexpectedly changed without retaining the initial placeholder')
...
for marker_text in (
    ...
    if marker_text not in repository_393:
        errors.append(f'3.9.5 installed-package version fallback missing: {marker_text}')
if 'throw new FileNotFoundException("The LocalGPT project file used for source-backed version reconciliation was not found."' in repository_393:
    errors.append('3.9.5 installed runtime still hard-fails when src/LocalGPT/LocalGPT.csproj is absent')

paths_393 = read('src/LocalGPT/Program.ServiceRegistration.cs')
if 'ResolveSafeCurrentDirectory()' not in paths_393:
    errors.append('3.9.5 safe current-directory resolver is missing')
for rel in (
    ...
    if 'Directory.GetCurrentDirectory()' in read(rel):
        errors.append(f'3.9.5 runtime current-directory dependency remains in {rel}')
...
for marker_text in (
    ...
    if marker_text not in release_ps:
        errors.append(f'3.9.5 release source-identity contract missing: {marker_text}')
if "Join-Path $versionDirectory 'SOURCE-SHA256.txt'" not in release_ps:
    errors.append('3.9.5 final release bundle does not persist source identity')
if "bundleSourceFingerprint" not in release_ps:
    errors.append('3.9.5 complete-bundle reuse does not validate source identity')
...
for marker_text in ('"llama3.1-8b": "llama3.1:8b"', '"deepseek-r1-32b": "deepseek-r1:32b"', '"qwen2.5-coder-32b": "qwen2.5-coder:32b"', '"qwen3.5-9b": "qwen3.5:9b"'):
    if provider_doc_393.count(marker_text) != 3:
        errors.append(f'3.9.5 expected Ollama knowledge alias is not present in all three platform profiles: {marker_text}')
...
if 'var installedBase = installed.Split' in setup_393 or 'var requestedBase = requested.Split' in setup_393:
    errors.append('3.9.5 installed-model matching still collapses distinct provider tag/size variants')
for rel in ('src/LocalGPT/Services/LocalPathExplorerService.cs', 'src/LocalGPT/Services/ProjectMaintenanceService.BuildReview.cs'):
    if 'Environment.CurrentDirectory' in read(rel):
        errors.append(f'3.9.5 runtime service still depends directly on Environment.CurrentDirectory: {rel}')
...
for marker_text in ('NullLogger.Instance', 'creationFailureReported', 'file logging was disabled'):
    if marker_text not in file_logger_provider_393:
        errors.append(f'3.9.5 file-logger construction defense missing: {marker_text}')

if errors:
    print('LocalGPT 3.9.5 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.9.5 source audit passed.')
