...
for rel in (
    ...
    req(rel, '<Version>3.5.9</Version>')
req('docs/docfx.json', '"localgptVersion": "3.5.9"')
req('docs/pdf/toc.yml', 'LocalGPT-3.5.9.pdf')
req('src/LocalGPT/Components/App.razor', 'localgpt-chat-ui.js?v=3.5.9')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs', 'LocalGPT/3.5.9')
req('RELEASE.md', 'CHANGELOG-v3.5.9-MACOS-COORDINATOR-NATIVE-PACKAGING.md')
req('RELEASE.md', 'VALIDATION-v3.5.9-source.md')
text('CHANGELOG-v3.5.9-MACOS-COORDINATOR-NATIVE-PACKAGING.md')
text('VALIDATION-v3.5.9-source.md')
...
# 3.5.9 macOS full-coordinator and native application packaging repairs.
for marker in (
    "'macOS'   { return @('osx-x64', 'osx-arm64', 'linux-x64', 'linux-arm64', 'win-x64', 'win-x86', 'win-arm64') }",
    'macOS is the full release coordinator',
    "-MacIconSource (Join-Path $root 'src/LocalGPT/wwwroot/android-chrome-512x512.png')",
    "-DmgBackgroundPath (Join-Path $root 'build/assets/LocalGPT-dmg-background.png')",
):
    if marker not in build:
        errors.append(f'Build-Release.ps1 missing 3.5.9 macOS coordinator marker: {marker}')
req('src/LocalGPT/LocalGPT.csproj', '<ApplicationIcon>wwwroot\\favicon.ico</ApplicationIcon>')
req('src/LocalGPTInstallerConsole/LocalGPTInstallerConsole.csproj', '<ApplicationIcon>..\\LocalGPT\\wwwroot\\favicon.ico</ApplicationIcon>')
text('build/assets/LocalGPT-dmg-background.png')
for marker in (
    ...
    if marker not in native:
        errors.append(f'NativeReleasePackaging.ps1 missing 3.5.9 macOS packaging marker: {marker}')
...
for forbidden in ('/usr/bin/open "http://127.0.0.1"',):
    if forbidden in native:
        errors.append(f'NativeReleasePackaging.ps1 retains broken bare-loopback launcher fallback: {forbidden}')
...
if any(x > 9 for x in (5, 9)):
    errors.append('version violates one-digit minor/patch policy')
if errors:
    print('LocalGPT 3.5.9 static release audit FAILED:')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)
print('LocalGPT 3.5.9 static release audit passed.')
