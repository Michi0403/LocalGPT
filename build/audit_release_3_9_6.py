...
try:
    ...
    if meta.get('localgptVersion')!='3.9.6': errors.append('docs/docfx.json localgptVersion != 3.9.6')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.9.6**'),('docs/pdf/toc.yml','LocalGPT-3.9.6.pdf'),
    ('RELEASE.md','# LocalGPT 3.9.6'),('CHANGELOG-v3.9.6-PROVIDER-MODEL-DISCOVERY-COUNCIL-BINDING-REPAIR.md','provider model discovery and Council binding repair'),
    ('VALIDATION-v3.9.6-source.md','# LocalGPT 3.9.6 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.9.6'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.9.6')): req(rel,mark)
...
# 3.9.4 Windows PowerShell 5.1 documentation-cache regression repair retained in 3.9.6.
...
# 3.9.6 Debug/Release documentation PDF contract repair.
...
for marker_text in (
    ...
    if marker_text not in docs_395:
        errors.append(f'3.9.6 documentation PDF contract marker missing: {marker_text}')
for marker_text in (
    ...
    if marker_text not in targets_395:
        errors.append(f'3.9.6 Directory.Build.targets PDF contract marker missing: {marker_text}')
if '-RequirePdf `' not in read('Build-Release.ps1'):
    errors.append('3.9.6 release orchestration no longer explicitly requires the complete documentation PDF')
...
if re.search(r'(?m)^if \(Test-Path -LiteralPath \$sourceWebRoot -PathType Container\) \{\s*\n\s*\$sourcePdfPath', docs_395):
    errors.append('3.9.6 still contains an unconditional source-tree PDF assertion')
...
# 3.9.6 build-guard repair. Reproduce the maintained PowerShell guard predicates in
...
for marker_text in ('bool IsOllamaProfile(AiProviderBootstrapProfile profile);',):
    if marker_text not in interface_text:
        errors.append(f'3.9.6 provider classification ownership marker missing: {marker_text}')
...
for marker_text in ('Providers.IsOllamaProfile(profile)', 'FirstOrDefault(Providers.IsOllamaProfile)'):
    if marker_text not in panel_391:
        errors.append(f'3.9.6 Setup provider-classification delegation missing: {marker_text}')
if '.StartsWith("ollama-", StringComparison.OrdinalIgnoreCase)' in panel_391:
    errors.append('3.9.6 Setup component still owns an Ollama profile-prefix StartsWith check')
...
for marker_text in ('private const string JsonMediaType = "application/json";', 'Encoding.UTF8, JsonMediaType'):
    if marker_text not in canirun_391:
        errors.append(f'3.9.6 CanIRun constructor-literal guard repair missing: {marker_text}')

# 3.9.6 packaged-runtime setup repair: the launcher, logger, optional hardware probes,
...
for marker_text in ('ResolveProcessWorkingDirectory()', 'RepairInvalidCurrentDirectory()', 'Directory.SetCurrentDirectory(ResolveProcessWorkingDirectory())'):
    if marker_text not in paths:
        errors.append(f'3.9.6 process working-directory repair missing: {marker_text}')
program_392 = read('src/LocalGPT/Program.cs')
if 'LocalGptApplicationDataPaths.RepairInvalidCurrentDirectory()' not in program_392:
    errors.append('3.9.6 startup no longer repairs an invalid inherited current directory')
...
for marker_text in ('cd "$USER_DATA_DIR/runtime"', 'durable per-user runtime working directory'):
    if marker_text not in launcher_392:
        errors.append(f'3.9.6 macOS launcher working-directory repair missing: {marker_text}')
if 'cd "$APP" || { show_failure "The packaged application directory could not be opened:' in launcher_392:
    errors.append('3.9.6 macOS launcher still makes the replaceable application bundle its process working directory')
...
for marker_text in ('ResolveLogPath(_options)', 'LocalGptApplicationDataPaths.ResolveUserPath("logs", "LocalGPT.log")', 'Path.GetTempPath()', 'AppContext.BaseDirectory'):
    if marker_text not in file_logger_392:
        errors.append(f'3.9.6 file-logger runtime-path repair missing: {marker_text}')
if 'Directory.GetCurrentDirectory()' in file_logger_392:
    errors.append('3.9.6 FileLogger still depends on Directory.GetCurrentDirectory()')
...
if 'return LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory();' not in console_392:
    errors.append('3.9.6 bounded console still lacks the stable default working directory used by provider/model installs')
if 'return Environment.CurrentDirectory;' in console_392:
    errors.append('3.9.6 bounded console still uses Environment.CurrentDirectory as its blank-request default')
...
for marker_text in ('platformProbe.ProbeNvidiaGpusAsync', 'platformGpus = []', 'GetCpuNameAsync'):
    if marker_text not in hardware_392:
        errors.append(f'3.9.6 hardware inventory platform-boundary repair missing: {marker_text}')
if 'OperatingSystem.Is' in hardware_392 or 'Process.Start' in hardware_392:
    errors.append('3.9.6 HardwareInventoryService bypasses the maintained platform probe boundary')
...
if platform_392.count('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()') != 2:
    errors.append('3.9.6 platform hardware probes must both use the stable process working directory')
...
for marker_text in ('Optional hardware discovery was unavailable while building initial setup snapshot', 'provider, Ollama, model, and recommendation controls remain usable.', 'hardware = [];'):
    if marker_text not in setup_392:
        errors.append(f'3.9.6 setup optional-hardware isolation missing: {marker_text}')
...
for marker_text in ('Start Ollama', 'Stop Ollama', 'Restart Ollama', 'Find hardware-fit models', 'Download / install model', 'Install model', 'Register endpoint in LocalGPT'):
    if marker_text not in panel_392:
        errors.append(f'3.9.6 unified setup workflow regressed required UI marker: {marker_text}')
for forbidden in ('recommendations.Take(24)', 'modelChoices.Take(32)'):
    if forbidden in panel_392:
        errors.append(f'3.9.6 unified setup workflow still hides fetched recommendations behind a Razor-only truncation: {forbidden}')
if '.Take(96)' in setup_392:
    errors.append('3.9.6 model-choice mapping still truncates CanIRun recommendations before the unified Setup form can offer each one')
canirun_392 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
if 'limit * 4' in canirun_392:
    errors.append('3.9.6 CanIRun parser retains the overflowing recommendation-limit multiplication that can collapse an unlimited policy to one result')
if 'result.Count >= 512' not in canirun_392:
    errors.append('3.9.6 CanIRun JSON traversal lost its explicit 512-object safety bound')


# 3.9.6 Windows/Mac runtime evidence repairs.
...
for marker_text in (
    ...
    if marker_text not in canirun_393:
        errors.append(f'3.9.6 CanIRun repair missing: {marker_text}')
if 'new("https://canirun.ai/api/recommend"' in canirun_393:
    errors.append('3.9.6 still starts CanIRun lookups at the redirecting non-www endpoint')
if 'AllowAutoRedirect = false' not in read('src/LocalGPT/Program.ServiceRegistration.cs'):
    errors.append('3.9.6 CanIRun HTTP client lost redirect-disabled transport policy')
...
for marker_text in (
    ...
    if marker_text not in setup_393:
        errors.append(f'3.9.6 setup isolation/model mapping repair missing: {marker_text}')
...
for marker_text in (
    ...
    if marker_text not in panel_393:
        errors.append(f'3.9.6 setup recovery/recommendation UI missing: {marker_text}')
if 'if (snapshot is null)' not in panel_393:
    errors.append('3.9.6 Setup loading contract unexpectedly changed without retaining the initial placeholder')
...
for marker_text in (
    ...
    if marker_text not in repository_393:
        errors.append(f'3.9.6 installed-package version fallback missing: {marker_text}')
if 'throw new FileNotFoundException("The LocalGPT project file used for source-backed version reconciliation was not found."' in repository_393:
    errors.append('3.9.6 installed runtime still hard-fails when src/LocalGPT/LocalGPT.csproj is absent')

paths_393 = read('src/LocalGPT/Program.ServiceRegistration.cs')
if 'ResolveSafeCurrentDirectory()' not in paths_393:
    errors.append('3.9.6 safe current-directory resolver is missing')
for rel in (
    ...
    if 'Directory.GetCurrentDirectory()' in read(rel):
        errors.append(f'3.9.6 runtime current-directory dependency remains in {rel}')
...
for marker_text in (
    ...
    if marker_text not in release_ps:
        errors.append(f'3.9.6 release source-identity contract missing: {marker_text}')
if "Join-Path $versionDirectory 'SOURCE-SHA256.txt'" not in release_ps:
    errors.append('3.9.6 final release bundle does not persist source identity')
if "bundleSourceFingerprint" not in release_ps:
    errors.append('3.9.6 complete-bundle reuse does not validate source identity')
...
for marker_text in ('"llama3.1-8b": "llama3.1:8b"', '"deepseek-r1-32b": "deepseek-r1:32b"', '"qwen2.5-coder-32b": "qwen2.5-coder:32b"', '"qwen3.5-9b": "qwen3.5:9b"'):
    if provider_doc_393.count(marker_text) != 3:
        errors.append(f'3.9.6 expected Ollama knowledge alias is not present in all three platform profiles: {marker_text}')
...
if 'var installedBase = installed.Split' in setup_393 or 'var requestedBase = requested.Split' in setup_393:
    errors.append('3.9.6 installed-model matching still collapses distinct provider tag/size variants')
for rel in ('src/LocalGPT/Services/LocalPathExplorerService.cs', 'src/LocalGPT/Services/ProjectMaintenanceService.BuildReview.cs'):
    if 'Environment.CurrentDirectory' in read(rel):
        errors.append(f'3.9.6 runtime service still depends directly on Environment.CurrentDirectory: {rel}')
...
for marker_text in ('NullLogger.Instance', 'creationFailureReported', 'file logging was disabled'):
    if marker_text not in file_logger_provider_393:
        errors.append(f'3.9.6 file-logger construction defense missing: {marker_text}')


# 3.9.6 provider model discovery, hardware, and persisted binding repair.
...
for marker_text in (
    ...
    if marker_text not in setup_396:
        errors.append(f'3.9.6 provider model discovery marker missing: {marker_text}')
if 'if (recommendations.Count == 0)' in setup_396:
    errors.append('3.9.6 provider model choices are still gated by CanIRun recommendations')
...
for marker_text in (
    ...
    if marker_text not in panel_396:
        errors.append(f'3.9.6 unified setup UI marker missing: {marker_text}')
if 'if (recommendations.Count == 0)' in panel_396 or 'recommendations.Take(' in panel_396 or 'modelChoices.Take(' in panel_396:
    errors.append('3.9.6 setup UI still gates or truncates provider model choices by CanIRun state')
...
for marker_text in ('CpuName', 'CanIRunHardwareName', 'LmStudioModelId', 'ModelCatalogUrl', 'IsProviderInventory', 'IsProviderCatalogEntry', 'IsHardwareRecommended', 'CanCheckUpdate'):
    if marker_text not in models_396:
        errors.append(f'3.9.6 setup model contract missing: {marker_text}')
...
for marker_text in ('ProbeNvidiaGpusAsync', 'ProbeCpuNameAsync', 'ProbeSystemMemoryBytesAsync'):
    if marker_text not in hardware_iface_396:
        errors.append(f'3.9.6 hardware platform contract missing: {marker_text}')
...
for marker_text in ('Win32_Processor', 'machdep.cpu.brand_string', '/proc/cpuinfo', 'ProbeNvidiaGpusAsync'):
    if marker_text not in platform_396:
        errors.append(f'3.9.6 cross-platform hardware probe marker missing: {marker_text}')

canirun_396 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
for marker_text in ('device.CpuName', 'device.CanIRunHardwareName', 'LmStudioModelId', 'LocalGPT/3.9.6'):
    if marker_text not in canirun_396:
        errors.append(f'3.9.6 CanIRun hardware-aware discovery marker missing: {marker_text}')
if 'throw new InvalidDataException("The CanIRun.ai recommendation does not expose a known-safe Ollama install mapping.' in setup_396:
    errors.append('3.9.6 reintroduced exception-driven control flow for an ordinary unmapped CanIRun model')
...
for marker_text in ('ResolveEquivalentCandidate', 'ProviderKindsEquivalent', 'EndpointsEquivalent', 'ModelNamesEquivalent', 'NormalizeModelToken'):
    if marker_text not in identity_396:
        errors.append(f'3.9.6 provider binding reconciliation marker missing: {marker_text}')
if "Split(':', 2)[0]" in identity_396 or 'Split(\':\')[0]' in identity_396:
    errors.append('3.9.6 provider identity reconciliation collapses concrete model variants')
for rel in (
    ...
    if 'ResolveEquivalentCandidate' not in read(rel):
        errors.append(f'3.9.6 provider binding reconciliation not wired into {rel}')
...
if errors:
    print('LocalGPT 3.9.6 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.9.6 source audit passed.')
