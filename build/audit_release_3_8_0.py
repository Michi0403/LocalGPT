...
try:
    ...
    if meta.get('localgptVersion')!='3.8.0': errors.append('docs/docfx.json localgptVersion != 3.8.0')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.8.0**'),('docs/pdf/toc.yml','LocalGPT-3.8.0.pdf'),
    ('RELEASE.md','# LocalGPT 3.8.0'),('CHANGELOG-v3.8.0-NOTARY-ORCHESTRATION-RECOVERY.md','Notary orchestration recovery'),
    ('VALIDATION-v3.8.0-source.md','# LocalGPT 3.8.0 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.8.0'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.8.0')): req(rel,mark)
...
if errors:
    print('LocalGPT 3.8.0 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.8.0 source audit passed.')
