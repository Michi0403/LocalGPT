...
for relative in (
    ...
    version(relative, "4.1.8")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.1.8"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.1.8.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.1.8 Documentation"),
    ("docs/index.md", "**Version 4.1.8**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.1.8"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.1.8"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.1.8"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.1.8"),
...
for relative, text417, tokens in (
    ...
    ("InitialSetupAssistantPanel.razor.css", setup_css_417, ("height: 24rem", "overflow: auto")),
...
try:
    ...
    if len(localization_en_417) != 2256 or len(localization_de_417) != 2256:
        failures.append(f"4.1.8 localization parity: expected 2256 keys each, got {len(localization_en_417)}/{len(localization_de_417)}")
except Exception as exc:
...
canirun_418 = read("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs")
setup_418 = read("src/LocalGPT/Services/InitialSetupAssistantService.cs")
provider_ui_418 = read("src/LocalGPT/Components/Shared/ProviderRuntimeManagementPanel.razor")
provider_css_418 = read("src/LocalGPT/Components/Shared/ProviderRuntimeManagementPanel.razor.css")
provider_docs_418 = read("docs/reference/ai-provider-installation.md")
for token in (
    'new("https://www.canirun.ai/api/models"',
    'new("https://www.canirun.ai/api/compatibility"',
    'LoadCatalogCompatibilityAsync',
    'Expanded CanIRun.ai hardware comparison',
):
    if token not in canirun_418:
        failures.append(f"4.1.8 CanIRun full-catalog comparison regressed: missing {token!r}")
for token in (
    'Take(16)',
    'values.Count >= 256',
    '.Take(256).ToList()',
    ...
    if token not in setup_418:
        failures.append(f"4.1.8 provider catalog/mapping contract regressed: missing {token!r}")
for token in (
    '"gemma4-e2b": "gemma4:e2b"',
    ...
    if provider_docs_418.count(token) < 3:
        failures.append(f"4.1.8 maintained Ollama alias set incomplete across platforms: {token!r}")
for token in (
    'Caption="@T("Bind address")" ColSpanMd="6"',
    'Caption="@T("Port")" ColSpanMd="6"',
    'Caption="@T("Keep alive (empty = Ollama default)")" ColSpanMd="6"',
    'Caption="@T("Parallel requests (0 = default)")" ColSpanMd="6"',
    'provider-runtime-delete-confirmation',
    'Enabled="@(!busy && permanentDeleteConfirmed)"',
):
    if token not in provider_ui_418:
        failures.append(f"4.1.8 provider workbench responsive/destructive contract regressed: missing {token!r}")
for token in ('::deep .dxbl-fl-cpt', '::deep .dxbl-fl-ctrl', '.provider-runtime-delete-confirmation'):
    if token not in provider_css_418:
        failures.append(f"4.1.8 provider workbench CSS contract regressed: missing {token!r}")

if failures:
    print("LocalGPT 4.1.8 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.1.8 release audit passed: full CanIRun catalog comparison, broader Ollama/provider catalog coverage and responsive provider controls are guarded while the 4.1.7 runtime-churn/cancellation protections and earlier architecture contracts remain intact.")
