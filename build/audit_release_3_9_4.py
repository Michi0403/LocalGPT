...
try:
    ...
    if meta.get('localgptVersion')!='3.9.4': errors.append('docs/docfx.json localgptVersion != 3.9.4')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.9.4**'),('docs/pdf/toc.yml','LocalGPT-3.9.4.pdf'),
    ('RELEASE.md','# LocalGPT 3.9.4'),('CHANGELOG-v3.9.4-POWERSHELL51-DOCUMENTATION-CACHE-REPAIR.md','Windows PowerShell 5.1 documentation-cache repair'),
    ('VALIDATION-v3.9.4-source.md','# LocalGPT 3.9.4 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.9.4'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.9.4')): req(rel,mark)
...
# 3.9.4 Windows PowerShell 5.1 documentation-cache regression repair.
...
if fixed_cache_join not in docs_cache:
    errors.append('3.9.4 documentation cache does not use nested two-argument Join-Path calls')
if legacy_cache_join in docs_cache:
    errors.append('3.9.4 documentation cache still uses PowerShell 6+ three-positional-argument Join-Path')
...
for marker_text in (
    ...
    if marker_text not in compat_394:
        errors.append(f'3.9.4 PowerShell 5.1 Join-Path guard missing: {marker_text}')
...
# 3.9.4 build-guard repair. Reproduce the maintained PowerShell guard predicates in
...
for marker_text in ('bool IsOllamaProfile(AiProviderBootstrapProfile profile);',):
    if marker_text not in interface_text:
        errors.append(f'3.9.4 provider classification ownership marker missing: {marker_text}')
...
for marker_text in ('Providers.IsOllamaProfile(profile)', 'FirstOrDefault(Providers.IsOllamaProfile)'):
    if marker_text not in panel_391:
        errors.append(f'3.9.4 Setup provider-classification delegation missing: {marker_text}')
if '.StartsWith("ollama-", StringComparison.OrdinalIgnoreCase)' in panel_391:
    errors.append('3.9.4 Setup component still owns an Ollama profile-prefix StartsWith check')
...
for marker_text in ('private const string JsonMediaType = "application/json";', 'Encoding.UTF8, JsonMediaType'):
    if marker_text not in canirun_391:
        errors.append(f'3.9.4 CanIRun constructor-literal guard repair missing: {marker_text}')

# 3.9.4 packaged-runtime setup repair: the launcher, logger, optional hardware probes,
...
for marker_text in ('ResolveProcessWorkingDirectory()', 'RepairInvalidCurrentDirectory()', 'Directory.SetCurrentDirectory(ResolveProcessWorkingDirectory())'):
    if marker_text not in paths:
        errors.append(f'3.9.4 process working-directory repair missing: {marker_text}')
program_392 = read('src/LocalGPT/Program.cs')
if 'LocalGptApplicationDataPaths.RepairInvalidCurrentDirectory()' not in program_392:
    errors.append('3.9.4 startup no longer repairs an invalid inherited current directory')
...
for marker_text in ('cd "$USER_DATA_DIR/runtime"', 'durable per-user runtime working directory'):
    if marker_text not in launcher_392:
        errors.append(f'3.9.4 macOS launcher working-directory repair missing: {marker_text}')
if 'cd "$APP" || { show_failure "The packaged application directory could not be opened:' in launcher_392:
    errors.append('3.9.4 macOS launcher still makes the replaceable application bundle its process working directory')
...
for marker_text in ('ResolveLogPath(_options)', 'LocalGptApplicationDataPaths.ResolveUserPath("logs", "LocalGPT.log")', 'Path.GetTempPath()', 'AppContext.BaseDirectory'):
    if marker_text not in file_logger_392:
        errors.append(f'3.9.4 file-logger runtime-path repair missing: {marker_text}')
if 'Directory.GetCurrentDirectory()' in file_logger_392:
    errors.append('3.9.4 FileLogger still depends on Directory.GetCurrentDirectory()')
...
if 'return LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory();' not in console_392:
    errors.append('3.9.4 bounded console still lacks the stable default working directory used by provider/model installs')
if 'return Environment.CurrentDirectory;' in console_392:
    errors.append('3.9.4 bounded console still uses Environment.CurrentDirectory as its blank-request default')
...
for marker_text in ('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()', 'Optional NVIDIA discovery was unavailable', 'platformGpus = []'):
    if marker_text not in hardware_392:
        errors.append(f'3.9.4 hardware inventory best-effort repair missing: {marker_text}')
...
if platform_392.count('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()') != 2:
    errors.append('3.9.4 platform hardware probes must both use the stable process working directory')
...
for marker_text in ('Optional hardware discovery was unavailable while building initial setup snapshot', 'provider, Ollama, model, and recommendation controls remain usable.', 'hardware = [];'):
    if marker_text not in setup_392:
        errors.append(f'3.9.4 setup optional-hardware isolation missing: {marker_text}')
...
for marker_text in ('Start Ollama', 'Stop Ollama', 'Restart Ollama', 'Fetch attributed recommendations', 'Download / install model', 'Install model', 'Register endpoint in LocalGPT'):
    if marker_text not in panel_392:
        errors.append(f'3.9.4 unified setup workflow regressed required UI marker: {marker_text}')
for forbidden in ('recommendations.Take(24)', 'modelChoices.Take(32)'):
    if forbidden in panel_392:
        errors.append(f'3.9.4 unified setup workflow still hides fetched recommendations behind a Razor-only truncation: {forbidden}')
if '.Take(96)' in setup_392:
    errors.append('3.9.4 model-choice mapping still truncates CanIRun recommendations before the unified Setup form can offer each one')
canirun_392 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
if 'limit * 4' in canirun_392:
    errors.append('3.9.4 CanIRun parser retains the overflowing recommendation-limit multiplication that can collapse an unlimited policy to one result')
if 'result.Count >= 512' not in canirun_392:
    errors.append('3.9.4 CanIRun JSON traversal lost its explicit 512-object safety bound')


# 3.9.4 Windows/Mac runtime evidence repairs.
...
for marker_text in (
    ...
    if marker_text not in canirun_393:
        errors.append(f'3.9.4 CanIRun repair missing: {marker_text}')
if 'new("https://canirun.ai/api/recommend"' in canirun_393:
    errors.append('3.9.4 still starts CanIRun lookups at the redirecting non-www endpoint')
if 'AllowAutoRedirect = false' not in read('src/LocalGPT/Program.ServiceRegistration.cs'):
    errors.append('3.9.4 CanIRun HTTP client lost redirect-disabled transport policy')
...
for marker_text in (
    ...
    if marker_text not in setup_393:
        errors.append(f'3.9.4 setup isolation/model mapping repair missing: {marker_text}')
...
for marker_text in (
    ...
    if marker_text not in panel_393:
        errors.append(f'3.9.4 setup recovery/recommendation UI missing: {marker_text}')
if 'if (snapshot is null)' not in panel_393:
    errors.append('3.9.4 Setup loading contract unexpectedly changed without retaining the initial placeholder')
...
for marker_text in (
    ...
    if marker_text not in repository_393:
        errors.append(f'3.9.4 installed-package version fallback missing: {marker_text}')
if 'throw new FileNotFoundException("The LocalGPT project file used for source-backed version reconciliation was not found."' in repository_393:
    errors.append('3.9.4 installed runtime still hard-fails when src/LocalGPT/LocalGPT.csproj is absent')

paths_393 = read('src/LocalGPT/Program.ServiceRegistration.cs')
if 'ResolveSafeCurrentDirectory()' not in paths_393:
    errors.append('3.9.4 safe current-directory resolver is missing')
for rel in (
    ...
    if 'Directory.GetCurrentDirectory()' in read(rel):
        errors.append(f'3.9.4 runtime current-directory dependency remains in {rel}')
...
for marker_text in (
    ...
    if marker_text not in release_ps:
        errors.append(f'3.9.4 release source-identity contract missing: {marker_text}')
if "Join-Path $versionDirectory 'SOURCE-SHA256.txt'" not in release_ps:
    errors.append('3.9.4 final release bundle does not persist source identity')
if "bundleSourceFingerprint" not in release_ps:
    errors.append('3.9.4 complete-bundle reuse does not validate source identity')
...
for marker_text in ('"llama3.1-8b": "llama3.1:8b"', '"deepseek-r1-32b": "deepseek-r1:32b"', '"qwen2.5-coder-32b": "qwen2.5-coder:32b"', '"qwen3.5-9b": "qwen3.5:9b"'):
    if provider_doc_393.count(marker_text) != 3:
        errors.append(f'3.9.4 expected Ollama knowledge alias is not present in all three platform profiles: {marker_text}')
...
if 'var installedBase = installed.Split' in setup_393 or 'var requestedBase = requested.Split' in setup_393:
    errors.append('3.9.4 installed-model matching still collapses distinct provider tag/size variants')
for rel in ('src/LocalGPT/Services/LocalPathExplorerService.cs', 'src/LocalGPT/Services/ProjectMaintenanceService.BuildReview.cs'):
    if 'Environment.CurrentDirectory' in read(rel):
        errors.append(f'3.9.4 runtime service still depends directly on Environment.CurrentDirectory: {rel}')
...
for marker_text in ('NullLogger.Instance', 'creationFailureReported', 'file logging was disabled'):
    if marker_text not in file_logger_provider_393:
        errors.append(f'3.9.4 file-logger construction defense missing: {marker_text}')

if errors:
    print('LocalGPT 3.9.4 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.9.4 source audit passed.')
