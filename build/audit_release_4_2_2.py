...
for relative in (
    ...
    version(relative, "4.2.2")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.2.2"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.2.2.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.2.2 Documentation"),
    ("docs/index.md", "**Version 4.2.2**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.2.2"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.2.2"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.2.2"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.2.2"),
...
# 4.2.2 metadata-backed console identity contract.
...
for token in (
    ...
    if token not in props_421:
        failures.append(f"4.2.2 repository/project metadata contract missing {token!r}")
...
for text, token, label in (
    ...
    if token not in text:
        failures.append(f"4.2.2 {label} is missing")
...
for token in (
    ...
    if token not in identity_421:
        failures.append(f"4.2.2 console identity helper missing {token!r}")
...
for forbidden in (
    ...
    if forbidden in identity_421:
        failures.append(f"4.2.2 console identity helper hardcodes project identity value {forbidden!r}")

if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.WriteStartupHeader();' not in app_program_421:
    failures.append("4.2.2 LocalGPT app startup does not print metadata-backed identity")
if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.WriteStartupHeader();' not in setup_program_421:
    failures.append("4.2.2 LocalGPT Setup startup does not print metadata-backed identity")
if 'private const string LocalGptRepo' in setup_program_421 or '"Michi0403/LocalGPT"' in setup_program_421:
    failures.append("4.2.2 LocalGPT Setup still duplicates the repository slug instead of deriving it from RepositoryUrl metadata")
if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.RepositorySlug' not in setup_program_421:
    failures.append("4.2.2 LocalGPT Setup does not consume the metadata-backed repository slug")


# 4.2.2 native-Ollama/OpenAI-compatible facade identity contract.
...
for forbidden in (
    ...
    if forbidden in runtime_422:
        failures.append(f"4.2.2 provider identity canonicalization still contains obsolete duplicate-provider policy {forbidden!r}")
for token in (
    ...
    if token not in runtime_422:
        failures.append(f"4.2.2 provider runtime alias collapse missing {token!r}")
for token in (
    ...
    if token not in identity_422:
        failures.append(f"4.2.2 provider identity reconciliation missing {token!r}")
if "ProviderModelKinds.OpenAICompatible" not in runtime_422 or "ProbeOpenAiCompatibleAsync" not in runtime_422:
    failures.append("4.2.2 accidentally removed independent OpenAI-compatible provider support")
if "AddCandidate(candidates, candidate);" not in runtime_422:
    failures.append("4.2.2 no longer preserves unmatched OpenAI-compatible models")

if failures:
    print("LocalGPT 4.2.2 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.2.2 release audit passed: native Ollama and its matching OpenAI-compatible /v1 facade are canonicalized to one provider/model identity without removing independent OpenAI-compatible providers; legacy facade bindings reconcile safely; metadata-backed console identity, live Ollama catalog/CanIRun workbench behavior, and prior runtime/signing protections remain intact.")
