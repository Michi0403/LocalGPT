...
try:
    ...
    if data.get('build',{}).get('globalMetadata',{}).get('localgptVersion')!='3.6.6':
        errors.append('docs/docfx.json localgptVersion is not 3.6.6')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','LocalGPT-3.6.6.pdf')
req('docs/index.md','**Version 3.6.6**')
req('RELEASE.md','# LocalGPT 3.6.6')
req('CHANGELOG-v3.6.6-DOCFX-PDF-CACHE-FALLBACK-REPAIR.md','Version advanced from 3.6.5 to 3.6.6.')
req('VALIDATION-v3.6.6-source.md','# LocalGPT 3.6.6 source validation')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.6.6')
req('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.6.6')
...
# Preserve 3.6.5 macOS architecture hardening.
...
if errors:
    print('LocalGPT 3.6.6 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('LocalGPT 3.6.6 source audit passed.')
