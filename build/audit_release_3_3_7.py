...
for project in [
    ...
    require('<Version>3.3.7</Version>' in read(project), f'{project} is not 3.3.7')
...
require('pdfFileName: LocalGPT-3.3.7.pdf' in read('docs/pdf/toc.yml'), 'PDF marker is not 3.3.7')
require('"localgptVersion": "3.3.7"' in read('docs/docfx.json'), 'DocFX marker is not 3.3.7')
require((root / 'CHANGELOG-v3.3.7-POWERSHELL-PARSER-PREFLIGHT.md').is_file(), '3.3.7 changelog missing')
require((root / 'VALIDATION-v3.3.7-source.md').is_file(), '3.3.7 validation missing')
require('# LocalGPT 3.3.7' in read('RELEASE.md'), 'RELEASE.md is not 3.3.7')
...
if failures:
    raise SystemExit('LocalGPT 3.3.7 static release audit failed:\n - ' + '\n - '.join(failures))
print('LocalGPT 3.3.7 static release audit passed.')
