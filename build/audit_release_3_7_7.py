...
try:
    ...
    if data.get('build',{}).get('globalMetadata',{}).get('localgptVersion')!='3.7.7':
        errors.append('docs/docfx.json localgptVersion is not 3.7.7')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','LocalGPT-3.7.7.pdf')
req('docs/index.md','**Version 3.7.7**')
req('RELEASE.md','# LocalGPT 3.7.7')
req('CHANGELOG-v3.7.7-NOTARY-KEYCHAIN-REGRESSION-REPAIR.md','html-browser-chunked')
req('VALIDATION-v3.7.7-source.md','# LocalGPT 3.7.7 source validation')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.7.7')
req('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.7.7')
...
# 3.7.7 regression: the release/development validators must accept the adaptive chunked browser PDF
# and still apply the same page/API completeness checks instead of bypassing them.
for marker in (
    ...
    'The LocalGPT documentation PDF omitted generated API pages.',
...
local_dev=text('Build-LocalDevelopment.ps1')
for marker in (
    '$browserBackedPdfModes = @("html-browser-print", "html-browser-print-compatibility", "html-browser-chunked")',
    '[string]$status.pdfMode -in $browserBackedPdfModes',
    'The LocalGPT documentation PDF omitted generated API pages.',
):
    if marker not in local_dev: errors.append(f'Build-LocalDevelopment.ps1 missing chunked-PDF validator marker: {marker}')
...
for marker in (
    ...
    if marker not in render_guard: errors.append(f'LocalGPT render-mode audit missing routed-page guard: {marker}')
...
for marker in ('Assert-MacNotaryCredentialsUsable', 'notarytool submit $ArtifactPath @notaryArgs --wait --timeout $timeout', 'No Keychain password item found'):
    if marker not in native: errors.append(f'build/NativeReleasePackaging.ps1 missing notary regression marker: {marker}')
if "-SelectedRuntimes @($Rid)" not in build: errors.append('Build-Release.ps1 missing per-macOS-RID trust revalidation')

if errors:
    print('LocalGPT 3.7.7 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('LocalGPT 3.7.7 source audit passed.')
