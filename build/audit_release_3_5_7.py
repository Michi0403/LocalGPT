...
for rel in (
    ...
    req(rel, '<Version>3.5.7</Version>')
req('docs/docfx.json', '"localgptVersion": "3.5.7"')
req('docs/pdf/toc.yml', 'LocalGPT-3.5.7.pdf')
req('src/LocalGPT/Components/App.razor', 'localgpt-chat-ui.js?v=3.5.7')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs', 'LocalGPT/3.5.7')
req('RELEASE.md', 'CHANGELOG-v3.5.7-WINDOWS-WSL-LINUX-RELEASE.md')
req('RELEASE.md', 'VALIDATION-v3.5.7-source.md')
text('CHANGELOG-v3.5.7-WINDOWS-WSL-LINUX-RELEASE.md')
text('VALIDATION-v3.5.7-source.md')
...
if any(x > 9 for x in (5, 7)):
    errors.append('version violates one-digit minor/patch policy')
if errors:
    print('LocalGPT 3.5.7 static release audit FAILED:')
    for error in errors:
        print(' -', error)
    raise SystemExit(1)
print('LocalGPT 3.5.7 static release audit passed.')
