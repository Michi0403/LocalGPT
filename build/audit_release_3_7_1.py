...
try:
    ...
    if data.get('build',{}).get('globalMetadata',{}).get('localgptVersion')!='3.7.1':
        errors.append('docs/docfx.json localgptVersion is not 3.7.1')
except Exception as exc: errors.append(f'docs/docfx.json JSON parse failed: {exc}')
req('docs/pdf/toc.yml','LocalGPT-3.7.1.pdf')
req('docs/index.md','**Version 3.7.1**')
req('RELEASE.md','# LocalGPT 3.7.1')
req('CHANGELOG-v3.7.1-EMBEDDED-COMPRESSED-DOCUMENTATION-PDF.md','Version advanced from 3.7.0 to 3.7.1')
req('VALIDATION-v3.7.1-source.md','# LocalGPT 3.7.1 source validation')
req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.7.1')
req('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.7.1')
...
# 3.7.1/3.3.1 release-size and resume invariants.
build=text('Build-Release.ps1')
for marker in (
...
for marker in ('FUTURE2_DOCUMENTATION_PDF_MAX_BYTES','Ensure-', 'ghostscript-screen-optimized','browserPdfTimeoutMilliseconds','maximumSanePdfBytes','268435456L','cached-validated-pdf','brewPath install ghostscript','runtimePdfPublished = Test-Path -LiteralPath (Join-Path $publishRoot $pdfName)'):
...
if errors:
    print('LocalGPT 3.7.1 static release audit FAILED:')
    for e in errors: print(' -',e)
    raise SystemExit(1)
print('LocalGPT 3.7.1 source audit passed.')
