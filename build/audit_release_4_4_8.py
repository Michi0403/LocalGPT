...
def main() -> int:
    ...
    release_markers = {
        ...
        "CHANGELOG-v4.4.8-KERNEL-CREATURE-ASCII-ANIMATIONS.md": f"# LocalGPT {VERSION}",
        "VALIDATION-v4.4.8-source.md": f"# LocalGPT {VERSION} source validation",
        "docs/index.md": f"**Version {VERSION}**",
        "docs/docfx.json": f'"localgptVersion": "{VERSION}"',
        "docs/pdf/toc.yml": "LocalGPT-4.4.8.pdf",
        "src/LocalGPT/Components/App.razor": "localgpt-game-console.js?v=4.4.8",
    }
    ...
    tournament_start = doom_team.find('Key = "kernel-creature-tournament"')
    tournament_end = doom_team.find('Key = "ascii-doom-council-adventure"', tournament_start)
    if tournament_start < 0 or tournament_end < 0:
        fail("Kernel Creature Tournament seed could not be isolated")
    tournament = doom_team[tournament_start:tournament_end]
    require(tournament, 'PreferredCapabilities = [AsciiChatTextService.RequiredSurfaceCapability]', "Kernel tournament required ASCII surface")
    require(tournament, 'Key = "arena-lineup-animation"', "Kernel tournament lineup animation")
    require(tournament, 'Key = "arena-round-animation"', "Kernel tournament round animation")
    require(tournament, '```ascii-sequence', "Kernel tournament transcript animation format")
    require(tournament, '--- frame ---', "Kernel tournament frame separator")
    require(tournament, 'never a new ruling', "Kernel tournament animation authority boundary")
    require(tournament, 'Tournament state remains text/transcript-owned', "Kernel tournament transcript authority")
    ...
    normalized_game_js = game_js.replace("\r\n", "\n").replace("\r", "\n")
    game_js_digest = hashlib.sha256(normalized_game_js.encode("utf-8")).hexdigest()
    diagnostics_manifest = read("build/javascript-diagnostics-files.sha256")
    require(
        diagnostics_manifest,
        f"{game_js_digest}  src/LocalGPT/wwwroot/js/localgpt-game-console.js",
        "reviewed JavaScript diagnostics manifest",
    )
    ...
    require(bootstrap_service, "if (runtimeClassKeys.Length == 0)", "surface-only ASCII Council handling")
    require(bootstrap_service, "will use transcript-owned ASCII presentation only", "surface-only ASCII diagnostic")
    ...
    require(team_config, "private const int CurrentSeedVersion = 31;", "Council system seed refresh")
    ...
    require(ascii_text, "IReadOnlyList<CouncilLiveParticipantActivitySnapshot>? participantActivities = null", "live Council ASCII sequence extraction")
    require(ascii_text, ".OrderByDescending(item => item.UpdatedAtUtc)", "newest live Council animation preference")
    require(console, "AsciiText.ExtractSequenceFrames(ConversationMessages, CouncilParticipantActivities)", "live Council animation terminal wiring")
    for marker_value in (
    ...
    for relative in current_metadata:
        for stale_version in ("4.4.6", "4.4.7"):
            forbid(read(relative), stale_version, f"stale current-version metadata in {relative}")

    print(
        f"LocalGPT {VERSION} release audit passed: Kernel Creature Tournament live ASCII lineup/replay animation, transcript authority, surface-only Council handling, JavaScript diagnostics synchronization, Council-run game ownership, DevExpress terminal routing, configurable DOOM campaign, documentation assets, InteractiveServer routes and the restored 4.4.2 shell are preserved."
    )
    return 0
