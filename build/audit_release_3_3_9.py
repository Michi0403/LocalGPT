...
for project in [
    ...
    require('<Version>3.3.9</Version>' in read(project), f'{project} is not 3.3.9')
...
require('js/localgpt-chat-ui.js?v=3.3.9' in read('src/LocalGPT/Components/App.razor'), 'browser cache marker is not 3.3.9')
require('LocalGPT/3.3.9' in read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs'), 'outbound product marker is not 3.3.9')
require('pdfFileName: LocalGPT-3.3.9.pdf' in read('docs/pdf/toc.yml'), 'PDF marker is not 3.3.9')
require('"localgptVersion": "3.3.9"' in read('docs/docfx.json'), 'DocFX marker is not 3.3.9')
require((root / 'CHANGELOG-v3.3.9-DOCUMENTATION-ACCESSIBILITY-FAILFAST.md').is_file(), '3.3.9 changelog missing')
require((root / 'VALIDATION-v3.3.9-source.md').is_file(), '3.3.9 validation missing')
require('# LocalGPT 3.3.9' in read('RELEASE.md'), 'RELEASE.md is not 3.3.9')

require('lang="en"' in doc and 'DocFX modern API pages can omit the html language attribute' in doc, 'DocFX HTML lang repair is missing')
require('Assert-LocalGptGeneratedHtmlPreflight -SiteRoot $siteRoot' in doc, 'pre-PDF HTML validator call is missing')
require('--html-only' in read('.github/scripts/prepare-pages-artifact.py'), 'Pages validator HTML-only mode is missing')
require('htmlPreflightValidated = $htmlPreflightValidated' in doc, 'documentation status does not record HTML preflight')
require('pre-PDF HTML accessibility/link preflight' in read('Build-Release.ps1'), 'release documentation guard does not require HTML preflight')
require('pre-PDF HTML accessibility/link preflight' in read('Build-LocalDevelopment.ps1'), 'development documentation guard does not require HTML preflight')
require('Documentation Python preflight' in read('build/Initialize-BuildPrerequisites.ps1'), 'Python prerequisite is not checked early')
require((root / 'CHANGELOG-v3.3.9-DOCUMENTATION-ACCESSIBILITY-FAILFAST.md').is_file(), '3.3.9 changelog missing')
...
if failures:
    raise SystemExit('LocalGPT 3.3.9 static release audit failed:\n - ' + '\n - '.join(failures))
print('LocalGPT 3.3.9 static release audit passed.')
