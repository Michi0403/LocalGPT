#!/usr/bin/env python3
"""Source-only release audit for LocalGPT 3.2.6 Remote Control workbench changes."""
...
for rel in ['src/LocalGPT/LocalGPT.csproj','src/LocalGPTInstallerConsole/LocalGPTInstallerConsole.csproj','src/LocalGPTWebviewWrapper/LocalGPTWebviewWrapper.csproj']:
    req(read(rel),'<Version>3.2.6</Version>',f'3.2.6 version in {rel}')
...
if failures:
    print('LocalGPT 3.2.6 source release audit failed:')
    for failure in failures: print('  -',failure)
    raise SystemExit(1)
print(f'LocalGPT 3.2.6 source release audit passed: {len(checks)} checks.')
