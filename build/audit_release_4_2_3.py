...
for relative in (
    ...
    version(relative, "4.2.3")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.2.3"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.2.3.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.2.3 Documentation"),
    ("docs/index.md", "**Version 4.2.3**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.2.3"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.2.3"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.2.3"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.2.3"),
...
if failures:
    print("LocalGPT 4.2.3 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.2.3 release audit passed: browser output/gamepad churn is bounded, Ollama health recovery verifies the local API, Council resource identity is protocol-independent and port-aware, TLS catalog failure preserves certificate validation with offline fallback, and 4.2.2 provider canonicalization plus prior release protections remain intact.")
