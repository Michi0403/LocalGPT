...
try:
    ...
    if meta.get('localgptVersion')!='3.9.7': errors.append('docs/docfx.json localgptVersion != 3.9.7')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.9.7**'),('docs/pdf/toc.yml','LocalGPT-3.9.7.pdf'),
    ('RELEASE.md','# LocalGPT 3.9.7'),('CHANGELOG-v3.9.7-COMPILE-REPAIR.md','compile repair'),
    ('VALIDATION-v3.9.7-source.md','# LocalGPT 3.9.7 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.9.7'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.9.7')): req(rel,mark)
...
# 3.9.4 Windows PowerShell 5.1 documentation-cache regression repair retained in 3.9.7.
docs_cache = read('build/Build-Documentation.ps1')
fixed_cache_join = "Join-Path (Join-Path $temporary 'site') $entry.Name"
legacy_cache_join = "Join-Path $temporary 'site' $entry.Name"
if fixed_cache_join not in docs_cache:
    errors.append('retained documentation cache does not use nested two-argument Join-Path calls')
if legacy_cache_join in docs_cache:
    errors.append('retained documentation cache still uses PowerShell 6+ three-positional-argument Join-Path')
compat_394 = read('build/Assert-PowerShellCompatibility.ps1')
for marker_text in (
    'AdditionalChildPath',
    '$joinPathCommands',
    ...
    if marker_text not in compat_394:
        errors.append(f'retained PowerShell 5.1 Join-Path guard missing: {marker_text}')


# 3.9.7 retains the Debug/Release documentation PDF contract repair.
docs_395 = read('build/Build-Documentation.ps1')
targets_395 = read('Directory.Build.targets')
for marker_text in (
    "if ($RequirePdf -or $pdfGenerated) {",
    'Runtime help-docs tree was not published: $sourceWebRoot',
    'Embedded LocalGPT documentation PDF was not published into the runtime help-docs tree: $sourcePdfPath',
    'Complete PDF generation was explicitly disabled; this HTML-only diagnostic build does not emit a fallback PDF.',
):
    if marker_text not in docs_395:
        errors.append(f'3.9.7 documentation PDF contract marker missing: {marker_text}')
for marker_text in (
    '<RequireLocalGptDocumentationPdf Condition="\'$(RequireLocalGptDocumentationPdf)\' == \'\' and \'$(Configuration)\' == \'Release\'">true</RequireLocalGptDocumentationPdf>',
    '<RequireLocalGptDocumentationPdf Condition="\'$(RequireLocalGptDocumentationPdf)\' == \'\'">false</RequireLocalGptDocumentationPdf>',
    '<LocalGptDocumentationPdfArgument Condition="\'$(RequireLocalGptDocumentationPdf)\' == \'true\'">-RequirePdf</LocalGptDocumentationPdfArgument>',
):
    if marker_text not in targets_395:
        errors.append(f'3.9.7 Directory.Build.targets PDF contract marker missing: {marker_text}')
if '-RequirePdf `' not in read('Build-Release.ps1'):
    errors.append('3.9.7 release orchestration no longer explicitly requires the complete documentation PDF')
# The final embedded-PDF assertion must not be a free-standing sourceWebRoot check anymore.
if re.search(r'(?m)^if \(Test-Path -LiteralPath \$sourceWebRoot -PathType Container\) \{\s*\n\s*\$sourcePdfPath', docs_395):
    errors.append('3.9.7 still contains an unconditional source-tree PDF assertion')
...
# 3.9.7 build-guard repair. Reproduce the maintained PowerShell guard predicates in
...
for marker_text in ('bool IsOllamaProfile(AiProviderBootstrapProfile profile);',):
    if marker_text not in interface_text:
        errors.append(f'3.9.7 provider classification ownership marker missing: {marker_text}')
...
for marker_text in ('Providers.IsOllamaProfile(profile)', 'FirstOrDefault(Providers.IsOllamaProfile)'):
    if marker_text not in panel_391:
        errors.append(f'3.9.7 Setup provider-classification delegation missing: {marker_text}')
if '.StartsWith("ollama-", StringComparison.OrdinalIgnoreCase)' in panel_391:
    errors.append('3.9.7 Setup component still owns an Ollama profile-prefix StartsWith check')
...
for marker_text in ('private const string JsonMediaType = "application/json";', 'Encoding.UTF8, JsonMediaType'):
    if marker_text not in canirun_391:
        errors.append(f'3.9.7 CanIRun constructor-literal guard repair missing: {marker_text}')

# 3.9.7 packaged-runtime setup repair: the launcher, logger, optional hardware probes,
...
for marker_text in ('ResolveProcessWorkingDirectory()', 'RepairInvalidCurrentDirectory()', 'Directory.SetCurrentDirectory(ResolveProcessWorkingDirectory())'):
    if marker_text not in paths:
        errors.append(f'3.9.7 process working-directory repair missing: {marker_text}')
program_392 = read('src/LocalGPT/Program.cs')
if 'LocalGptApplicationDataPaths.RepairInvalidCurrentDirectory()' not in program_392:
    errors.append('3.9.7 startup no longer repairs an invalid inherited current directory')
...
for marker_text in ('cd "$USER_DATA_DIR/runtime"', 'durable per-user runtime working directory'):
    if marker_text not in launcher_392:
        errors.append(f'3.9.7 macOS launcher working-directory repair missing: {marker_text}')
if 'cd "$APP" || { show_failure "The packaged application directory could not be opened:' in launcher_392:
    errors.append('3.9.7 macOS launcher still makes the replaceable application bundle its process working directory')
...
for marker_text in ('ResolveLogPath(_options)', 'LocalGptApplicationDataPaths.ResolveUserPath("logs", "LocalGPT.log")', 'Path.GetTempPath()', 'AppContext.BaseDirectory'):
    if marker_text not in file_logger_392:
        errors.append(f'3.9.7 file-logger runtime-path repair missing: {marker_text}')
if 'Directory.GetCurrentDirectory()' in file_logger_392:
    errors.append('3.9.7 FileLogger still depends on Directory.GetCurrentDirectory()')
...
if 'return LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory();' not in console_392:
    errors.append('3.9.7 bounded console still lacks the stable default working directory used by provider/model installs')
if 'return Environment.CurrentDirectory;' in console_392:
    errors.append('3.9.7 bounded console still uses Environment.CurrentDirectory as its blank-request default')
hardware_392 = read('src/LocalGPT/Services/HardwareInventoryService.cs')
for marker_text in ('platformProbe.ProbeNvidiaGpusAsync', 'platformGpus = []', 'GetCpuNameAsync'):
    if marker_text not in hardware_392:
        errors.append(f'3.9.7 hardware inventory platform-boundary repair missing: {marker_text}')
if 'OperatingSystem.Is' in hardware_392 or 'Process.Start' in hardware_392:
    errors.append('3.9.7 HardwareInventoryService bypasses the maintained platform probe boundary')
...
if platform_392.count('WorkingDirectory = LocalGptApplicationDataPaths.ResolveProcessWorkingDirectory()') != 2:
    errors.append('3.9.7 platform hardware probes must both use the stable process working directory')
...
for marker_text in ('Optional hardware discovery was unavailable while building initial setup snapshot', 'provider, Ollama, model, and recommendation controls remain usable.', 'hardware = [];'):
    if marker_text not in setup_392:
        errors.append(f'3.9.7 setup optional-hardware isolation missing: {marker_text}')
panel_392 = read('src/LocalGPT/Components/Shared/InitialSetupAssistantPanel.razor')
for marker_text in ('Start Ollama', 'Stop Ollama', 'Restart Ollama', 'Find hardware-fit models', 'Download / install model', 'Install model', 'Register endpoint in LocalGPT'):
    if marker_text not in panel_392:
        errors.append(f'3.9.7 unified setup workflow regressed required UI marker: {marker_text}')
for forbidden in ('recommendations.Take(24)', 'modelChoices.Take(32)'):
    if forbidden in panel_392:
        errors.append(f'3.9.7 unified setup workflow still hides fetched recommendations behind a Razor-only truncation: {forbidden}')
if '.Take(96)' in setup_392:
    errors.append('3.9.7 model-choice mapping still truncates CanIRun recommendations before the unified Setup form can offer each one')
canirun_392 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
if 'limit * 4' in canirun_392:
    errors.append('3.9.7 CanIRun parser retains the overflowing recommendation-limit multiplication that can collapse an unlimited policy to one result')
if 'result.Count >= 512' not in canirun_392:
    errors.append('3.9.7 CanIRun JSON traversal lost its explicit 512-object safety bound')


# 3.9.7 Windows/Mac runtime evidence repairs.
...
for marker_text in (
    ...
    if marker_text not in canirun_393:
        errors.append(f'3.9.7 CanIRun repair missing: {marker_text}')
if 'new("https://canirun.ai/api/recommend"' in canirun_393:
    errors.append('3.9.7 still starts CanIRun lookups at the redirecting non-www endpoint')
if 'AllowAutoRedirect = false' not in read('src/LocalGPT/Program.ServiceRegistration.cs'):
    errors.append('3.9.7 CanIRun HTTP client lost redirect-disabled transport policy')
...
for marker_text in (
    ...
    '"qwen3.5"',
    'Provider install ID needs review.',
    'Provider model discovery was unavailable while building setup choices',
    'One optional CanIRun.ai hardware lookup failed',
):
    if marker_text not in setup_393:
        errors.append(f'3.9.7 setup isolation/model mapping repair missing: {marker_text}')
...
for marker_text in (
    ...
    if marker_text not in panel_393:
        errors.append(f'3.9.7 setup recovery/recommendation UI missing: {marker_text}')
if 'if (snapshot is null)' not in panel_393:
    errors.append('3.9.7 Setup loading contract unexpectedly changed without retaining the initial placeholder')
...
for marker_text in (
    ...
    if marker_text not in repository_393:
        errors.append(f'3.9.7 installed-package version fallback missing: {marker_text}')
if 'throw new FileNotFoundException("The LocalGPT project file used for source-backed version reconciliation was not found."' in repository_393:
    errors.append('3.9.7 installed runtime still hard-fails when src/LocalGPT/LocalGPT.csproj is absent')

paths_393 = read('src/LocalGPT/Program.ServiceRegistration.cs')
if 'ResolveSafeCurrentDirectory()' not in paths_393:
    errors.append('3.9.7 safe current-directory resolver is missing')
for rel in (
    ...
    if 'Directory.GetCurrentDirectory()' in read(rel):
        errors.append(f'3.9.7 runtime current-directory dependency remains in {rel}')
...
for marker_text in (
    ...
    if marker_text not in release_ps:
        errors.append(f'3.9.7 release source-identity contract missing: {marker_text}')
if "Join-Path $versionDirectory 'SOURCE-SHA256.txt'" not in release_ps:
    errors.append('3.9.7 final release bundle does not persist source identity')
if "bundleSourceFingerprint" not in release_ps:
    errors.append('3.9.7 complete-bundle reuse does not validate source identity')
...
for marker_text in ('"llama3.1-8b": "llama3.1:8b"', '"deepseek-r1-32b": "deepseek-r1:32b"', '"qwen2.5-coder-32b": "qwen2.5-coder:32b"', '"qwen3.5-9b": "qwen3.5:9b"'):
    if provider_doc_393.count(marker_text) != 3:
        errors.append(f'3.9.7 expected Ollama knowledge alias is not present in all three platform profiles: {marker_text}')
...
if 'var installedBase = installed.Split' in setup_393 or 'var requestedBase = requested.Split' in setup_393:
    errors.append('3.9.7 installed-model matching still collapses distinct provider tag/size variants')
for rel in ('src/LocalGPT/Services/LocalPathExplorerService.cs', 'src/LocalGPT/Services/ProjectMaintenanceService.BuildReview.cs'):
    if 'Environment.CurrentDirectory' in read(rel):
        errors.append(f'3.9.7 runtime service still depends directly on Environment.CurrentDirectory: {rel}')
...
for marker_text in ('NullLogger.Instance', 'creationFailureReported', 'file logging was disabled'):
    if marker_text not in file_logger_provider_393:
        errors.append(f'3.9.7 file-logger construction defense missing: {marker_text}')


# 3.9.7 provider model discovery, hardware, and persisted binding repair.
setup_396 = read('src/LocalGPT/Services/InitialSetupAssistantService.cs')
for marker_text in (
    'SearchProviderCatalogAsync',
    'Installed model reported by the selected provider.',
    'Knowledge-backed provider catalog mapping.',
    'CanIRun.ai hardware-fit discovery mapped to the selected provider.',
    'Provider install ID needs review.',
    'IsHardwareRecommended = true',
    'LocalGPTProviderCatalog',
    'https://ollama.com/search?q=',
    'https://lmstudio.ai/models',
):
    if marker_text not in setup_396:
        errors.append(f'3.9.7 provider model discovery marker missing: {marker_text}')
if 'if (recommendations.Count == 0)' in setup_396:
    errors.append('3.9.7 provider model choices are still gated by CanIRun recommendations')

panel_396 = read('src/LocalGPT/Components/Shared/InitialSetupAssistantPanel.razor')
for marker_text in (
    'Local host hardware',
    'CPU / SoC',
    'CanIRun hardware match (optional catalog variant)',
    'Optional CanIRun.ai hardware-fit model discovery',
    'Provider models and hardware-fit discoveries',
    'Search official provider catalog',
    'Check / refresh model',
    'Start Ollama', 'Stop Ollama', 'Restart Ollama', 'Refresh Ollama / models',
):
    if marker_text not in panel_396:
        errors.append(f'3.9.7 unified setup UI marker missing: {marker_text}')
if 'if (recommendations.Count == 0)' in panel_396 or 'recommendations.Take(' in panel_396 or 'modelChoices.Take(' in panel_396:
    errors.append('3.9.7 setup UI still gates or truncates provider model choices by CanIRun state')

models_396 = read('src/LocalGPT/BusinessObjects/InitialSetupAssistantModels.cs')
for marker_text in ('CpuName', 'CanIRunHardwareName', 'LmStudioModelId', 'ModelCatalogUrl', 'IsProviderInventory', 'IsProviderCatalogEntry', 'IsHardwareRecommended', 'CanCheckUpdate'):
    if marker_text not in models_396:
        errors.append(f'3.9.7 setup model contract missing: {marker_text}')

hardware_iface_396 = read('src/LocalGPT/Interfaces/IHardwarePlatformProbeService.cs')
for marker_text in ('ProbeNvidiaGpusAsync', 'ProbeCpuNameAsync', 'ProbeSystemMemoryBytesAsync'):
    if marker_text not in hardware_iface_396:
        errors.append(f'3.9.7 hardware platform contract missing: {marker_text}')
platform_396 = read('src/LocalGPT/Services/HardwarePlatformProbeServices.cs')
for marker_text in ('Win32_Processor', 'machdep.cpu.brand_string', '/proc/cpuinfo', 'ProbeNvidiaGpusAsync'):
    if marker_text not in platform_396:
        errors.append(f'3.9.7 cross-platform hardware probe marker missing: {marker_text}')

canirun_396 = read('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs')
for marker_text in ('device.CpuName', 'device.CanIRunHardwareName', 'LmStudioModelId', 'LocalGPT/3.9.7'):
    if marker_text not in canirun_396:
        errors.append(f'3.9.7 CanIRun hardware-aware discovery marker missing: {marker_text}')
if 'throw new InvalidDataException("The CanIRun.ai recommendation does not expose a known-safe Ollama install mapping.' in setup_396:
    errors.append('3.9.7 reintroduced exception-driven control flow for an ordinary unmapped CanIRun model')

identity_396 = read('src/LocalGPT/BusinessObjects/ProviderModelModels.cs')
for marker_text in ('ResolveEquivalentCandidate', 'ProviderKindsEquivalent', 'EndpointsEquivalent', 'ModelNamesEquivalent', 'NormalizeModelToken'):
    if marker_text not in identity_396:
        errors.append(f'3.9.7 provider binding reconciliation marker missing: {marker_text}')
if "Split(':', 2)[0]" in identity_396 or 'Split(\':\')[0]' in identity_396:
    errors.append('3.9.7 provider identity reconciliation collapses concrete model variants')
for rel in (
    'src/LocalGPT/Services/ProviderModelRuntimeService.cs',
    'src/LocalGPT/Services/MultiModelCouncilService.cs',
    'src/LocalGPT/Services/MultiModelCouncilService.ParticipantSelection.cs',
    'src/LocalGPT/Components/Pages/CouncilTeams.PersistenceAndPolicy.razor.cs',
    'src/LocalGPT/Components/Pages/Chat.CouncilRunControlsAndBenchmarks.razor.cs',
    'src/LocalGPT/Components/Pages/ModelCouncil.razor.cs',
):
    if 'ResolveEquivalentCandidate' not in read(rel):
        errors.append(f'3.9.7 provider binding reconciliation not wired into {rel}')

# Preserve established component/service ownership counts.
razor_count = 0
for p in (ROOT/'src'/'LocalGPT').rglob('*.razor'):
    razor_count += p.read_text(encoding='utf-8-sig', errors='replace').count('@rendermode InteractiveServer')
if razor_count != 15:
    errors.append(f'expected 15 direct @rendermode InteractiveServer boundaries, found {razor_count}')
hosted_count = 0
for p in (ROOT/'src'/'LocalGPT').rglob('*.cs'):
    hosted_count += len(re.findall(r'AddHostedService\s*<', p.read_text(encoding='utf-8-sig', errors='replace')))
if hosted_count != 8:
    errors.append(f'expected 8 AddHostedService<T> registrations, found {hosted_count}')

# 3.9.7 compiler-log repair: readonly structs must not let LINQ lambdas capture implicit this,
# and BuildHardwareListAsync must keep configured-profile CPU state distinct from later local detection.
provider_models = read('src/LocalGPT/BusinessObjects/ProviderModelModels.cs')
for marker_text in (
    'var identity = this;',
    '.Where(candidate => identity.ProviderKindsEquivalent(',
    '.Where(candidate => identity.EndpointsEquivalent(',
    '.Where(candidate => identity.ModelNamesEquivalent(',
    'var modelIdentity = this;',
    '.Where(candidate => modelIdentity.ModelNamesEquivalent(',
):
    if marker_text not in provider_models:
        errors.append(f'3.9.7 readonly-struct LINQ capture repair missing: {marker_text}')
for forbidden_text in (
    '.Where(candidate => ProviderKindsEquivalent(',
    '.Where(candidate => EndpointsEquivalent(',
    '.Where(candidate => ModelNamesEquivalent(',
):
    if forbidden_text in provider_models:
        errors.append(f'3.9.7 reintroduced implicit readonly-struct this capture: {forbidden_text}')
setup_service = read('src/LocalGPT/Services/InitialSetupAssistantService.cs')
for marker_text in (
    'var profileCpuName = string.IsNullOrWhiteSpace(profile.CpuName)',
    'CpuName = profileCpuName,',
    'var cpuName = cpu?.Name ?? await hardwareInventory.GetCpuNameAsync',
):
    if marker_text not in setup_service:
        errors.append(f'3.9.7 CPU local-scope compile repair missing: {marker_text}')
if re.search(r'foreach \(var profile in profiles\).*?var cpuName = string\.IsNullOrWhiteSpace\(profile\.CpuName\)', setup_service, re.S):
    errors.append('3.9.7 reintroduced configured-profile cpuName declaration that conflicts with later detected cpuName')

if errors:
    print('LocalGPT 3.9.7 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.9.7 source audit passed.')
