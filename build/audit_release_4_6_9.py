...
def main():
    ...
    req('CHANGELOG-v4.6.9-CHAT-WORKSPACE-DRAG-DROP.md', f'# LocalGPT {VERSION}')
    req('VALIDATION-v4.6.9-source.md', f'# LocalGPT {VERSION} source validation')
    req('src/LocalGPT/Components/App.razor', 'localgpt-game-console.js?v=4.6.9')
    req('src/LocalGPT/Components/App.razor', 'localgpt-chat-ui.js?v=4.6.9')
    req('src/LocalGPT/Services/InitialSetupAssistantService.cs', 'LocalGPT/4.6.9')
    req('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs', 'LocalGPT/4.6.9')
    req('docs/index.md', '**Version 4.6.9**')
    req('docs/docfx.json', '"localgptVersion": "4.6.9"')
    req('docs/pdf-cover.html', 'LocalGPT 4.6.9 Documentation')
    req('docs/pdf/toc.yml', 'LocalGPT-4.6.9.pdf')
    ...
    # 4.6.9: Chat upload intake is idle-invisible and browser-owned during external file drag.
    advisor = 'src/LocalGPT/Components/Shared/UploadProcessingAdvisor.razor'
    for marker in [
        '@implements IAsyncDisposable', '<DxPopup', '<DxFormLayout', '<DxAccordion',
        'localGptChatUi.registerWorkspaceDropTarget', 'ExternalWorkspaceDropAcceptedAsync',
        'ExternalWorkspaceDropFailedAsync', 'Use recommendation in Chat',
        'Ask suggested Council team', 'Request independent Council review', 'Approve promotion']:
        req(advisor, marker)
    for marker in [
        '<DxFileInput', '<DxGridLayout', 'upload-processing-advisor-grid',
        'What should LocalGPT help you achieve?', 'UploadMode.OnButtonClick',
        'CreateWorkspaceFromStreamsAsync', '<div class="upload-processing-advisor"', 'Select File']:
        forbid(advisor, marker)
    ...
    # Keep the generic DevExpress GridLayout misuse guard added by 4.6.7 even though the idle upload surface no longer renders a grid.
    ...
    drop_controller = 'src/LocalGPT/Controller/ChatUploadController.cs'
    for marker in ['[HttpPost("drop")]', 'Request.ReadFormAsync', 'catalog.MaxSingleFileBytes',
                   'catalog.MaxTotalFileBytes', 'CreateWorkspaceFromStreamsAsync', 'file.OpenReadStream']:
        req(drop_controller, marker)
    forbid(drop_controller, 'ReadAllBytes')
    forbid(drop_controller, 'CopyToAsync(new MemoryStream')

    chat_js = 'src/LocalGPT/wwwroot/js/localgpt-chat-ui.js'
    for marker in ['registerWorkspaceDropTarget', 'localgpt-workspace-drop-overlay', 'new FormData()',
                   'fetch(String(state.options.uploadUrl', 'ExternalWorkspaceDropAcceptedAsync', 'event.preventDefault()']:
        req(chat_js, marker)
    drop_css = 'src/LocalGPT/wwwroot/css/site.css'
    for marker in ['.localgpt-workspace-drop-overlay', 'position: fixed;', 'pointer-events: none;',
                   '.localgpt-workspace-drop-overlay.uploading']:
        req(drop_css, marker)
    ...
    # 4.6.8 tournament/session correlation, pixel presentation and bounded rejoin remain intact.
    ...
    print(f'LocalGPT {VERSION} release audit passed: idle-invisible Chat drag/drop, bounded streamed quarantine intake, post-drop review, prior tournament/pixel/rejoin repairs, InteractiveServer topology and {checked} JS hashes are consistent.')
    return 0
