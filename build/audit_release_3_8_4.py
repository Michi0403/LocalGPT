...
try:
    ...
    if meta.get('localgptVersion')!='3.8.4': errors.append('docs/docfx.json localgptVersion != 3.8.4')
except Exception as exc: errors.append(f'docfx json parse failed: {exc}')
for rel,mark in (
    ('docs/index.md','**Version 3.8.4**'),('docs/pdf/toc.yml','LocalGPT-3.8.4.pdf'),
    ('RELEASE.md','# LocalGPT 3.8.4'),('CHANGELOG-v3.8.4-USER-DATA-PATH-CONTRACT.md','per-user data and first-boot path contract'),
    ('VALIDATION-v3.8.4-source.md','# LocalGPT 3.8.4 source validation'),
    ('src/LocalGPT/Components/App.razor','localgpt-chat-ui.js?v=3.8.4'),
    ('src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs','LocalGPT/3.8.4')): req(rel,mark)
...
if errors:
    print('LocalGPT 3.8.4 static release audit FAILED:')
    for e in errors: print(' -',e)
    sys.exit(1)
print('LocalGPT 3.8.4 source audit passed.')
