...
for relative in (
    ...
    version(relative, "4.2.1")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.2.1"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.2.1.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.2.1 Documentation"),
    ("docs/index.md", "**Version 4.2.1**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.2.1"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.2.1"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.2.1"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.2.1"),
...
# 4.2.1 metadata-backed console identity contract.
...
for token in (
    ...
    if token not in props_421:
        failures.append(f"4.2.1 repository/project metadata contract missing {token!r}")
...
for text, token, label in (
    ...
    if token not in text:
        failures.append(f"4.2.1 {label} is missing")
...
for token in (
    ...
    if token not in identity_421:
        failures.append(f"4.2.1 console identity helper missing {token!r}")
...
for forbidden in (
    ...
    if forbidden in identity_421:
        failures.append(f"4.2.1 console identity helper hardcodes project identity value {forbidden!r}")

if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.WriteStartupHeader();' not in app_program_421:
    failures.append("4.2.1 LocalGPT app startup does not print metadata-backed identity")
if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.WriteStartupHeader();' not in setup_program_421:
    failures.append("4.2.1 LocalGPT Setup startup does not print metadata-backed identity")
if 'private const string LocalGptRepo' in setup_program_421 or '"Michi0403/LocalGPT"' in setup_program_421:
    failures.append("4.2.1 LocalGPT Setup still duplicates the repository slug instead of deriving it from RepositoryUrl metadata")
if 'global::ProjectConsoleIdentity.ConsoleProductIdentity.RepositorySlug' not in setup_program_421:
    failures.append("4.2.1 LocalGPT Setup does not consume the metadata-backed repository slug")

if failures:
    print("LocalGPT 4.2.1 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.2.1 release audit passed: metadata-backed app/setup console identity is guarded without duplicated project identity values;  live Ollama search expands official/community families through tags pages, stale profiles receive current offline aliases, CanIRun catalog comparison is independently bounded, the runtime workbench labels/layout are stable, and prior runtime/signing protections remain intact.")
