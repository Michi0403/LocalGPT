#!/usr/bin/env python3
"""Source-only release checks for LocalGPT 4.0.2."""
...
for rel, needle in (
    ...
    ("VALIDATION-v4.0.2-source.md", "# LocalGPT 4.0.2 source validation"),
    ("CHANGELOG-v4.0.2-MACOS-RUNTIME-IDENTITY-SETUP-RECOVERY.md", "macOS updater handoff"),
):
    require(rel, needle)
...
# Windows Ollama profile retains the direct fast HTTP path while release identity moves to 4.0.2.
...
# 4.0.2 package/runtime identity and macOS update lifecycle.
...
for needle in (
    ...
    'Rejecting stale runtime endpoint',
...
if ERRORS:
    print("LocalGPT 4.0.2 source audit FAILED:")
    for error in ERRORS:
        print(f" - {error}")
    sys.exit(1)
print(
    ...
    f"{len(profiles)} provider profiles, macOS runtime/update identity guarded."
)
