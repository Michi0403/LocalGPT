...
def main() -> int:
    ...
    release_markers = {
        ...
        "CHANGELOG-v4.4.5-ASCII-CAMPAIGN-UI-DOCS.md": f"# LocalGPT {VERSION}",
        "VALIDATION-v4.4.5-source.md": f"# LocalGPT {VERSION} source validation",
        "docs/index.md": f"**Version {VERSION}**",
        "docs/docfx.json": f'"localgptVersion": "{VERSION}"',
        "docs/pdf/toc.yml": "LocalGPT-4.4.5.pdf",
        "src/LocalGPT/Components/App.razor": "localgpt-game-console.js?v=4.4.5",
    }
    ...
    for relative in current_metadata:
        forbid(read(relative), "4.4.4", f"stale current-version metadata in {relative}")

    print(
        f"LocalGPT {VERSION} release audit passed: configurable ten-level ASCII DOOM runtime-class campaign, optional-human/runtime-control separation, explicit renderer-affine DevExpress console continuations, popup fit repair, rare documentation shooting-star sky, synchronized documentation assets/cache keys, macOS Installer title, Mermaid layout, InteractiveServer routes and the restored 4.4.2 shell are preserved."
    )
    return 0
