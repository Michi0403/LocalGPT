...
def main() -> int:
    ...
    req("CHANGELOG-v4.6.1-SERVICE-LIFETIME-REPAIR.md", f"# LocalGPT {VERSION}")
    req("VALIDATION-v4.6.1-source.md", f"# LocalGPT {VERSION} source validation")
    for marker in ["localgpt-game-console.js?v=4.6.1", "localgpt-chat-ui.js?v=4.6.1"]:
        req("src/LocalGPT/Components/App.razor", marker)
    req("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.6.1")
    req("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.6.1")
    req("docs/index.md", "**Version 4.6.1**")
    req("docs/docfx.json", '"localgptVersion": "4.6.1"')
    ...
    # 4.6.0 compiler/UI corrective guards retained in the 4.6.1 startup hotfix.
    ...
    # 4.6.1 startup lifetime repair: keep singleton ownership, but never capture scoped freshness directly.
    registration = "src/LocalGPT/Program.ServiceRegistration.cs"
    req(registration, "builder.Services.AddSingleton<IHumanCollaborationService, HumanCollaborationService>();")
    req(registration, "builder.Services.AddScoped<IKnowledgeFreshnessReviewService, KnowledgeFreshnessReviewService>();")
    collaboration = "src/LocalGPT/Services/HumanCollaborationService.cs"
    req(collaboration, "IServiceScopeFactory serviceScopeFactory")
    req(collaboration, "using var freshnessScope = serviceScopeFactory.CreateScope();")
    req(collaboration, "freshnessScope.ServiceProvider.GetRequiredService<IKnowledgeFreshnessReviewService>()")
    forbid(collaboration, "private readonly IKnowledgeFreshnessReviewService knowledgeFreshness;")
    forbid(collaboration, "IKnowledgeFreshnessReviewService knowledgeFreshness,")
    ...
    print(f"LocalGPT {VERSION} release audit passed: completed 4.6.1 startup lifetime repair while preserving the 4.6.0 compile/UI and freshness/rejoin/tournament/dropdown work, render topology and {checked} JS hashes are consistent.")
    return 0
