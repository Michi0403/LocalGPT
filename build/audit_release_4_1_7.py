...
for relative in (
    ...
    version(relative, "4.1.7")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.1.7"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.1.7.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.1.7 Documentation"),
    ("docs/index.md", "**Version 4.1.7**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.1.7"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.1.7"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.1.7"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.1.7"),
...
# 4.1.7 runtime churn, provider-storage and setup completion contract.
...
for relative, text417, tokens in (
    ...
    for token in tokens:
        if token not in text417:
            failures.append(f"4.1.7 contract regressed in {relative}: missing {token!r}")

if "while (!reader.EndOfStream)" in ollama_chat_417:
    failures.append("4.1.7 Ollama cancellation race regressed: streaming loop probes EndOfStream before its cancellable read")
...
if not cancellation_body:
    failures.append("4.1.7 cancellation audit cannot find LogCancellation")
elif "LogInformation(\n            exception," in cancellation_body or "LogInformation(exception" in cancellation_body:
    failures.append("4.1.7 cancellation audit: informational cancellation logging still includes an exception stack")
...
if "if (initializedNow)" not in initialize_body or "Database migration and deterministic initial data feed completed." not in initialize_body:
    failures.append("4.1.7 database audit: InitializeAsync no longer conditionally emits the completion event")
if 'logger.LogDebug("Starting boot-critical LocalGPT database initialization' in initialize_body:
    failures.append("4.1.7 database audit: idempotent initialization calls still emit the old Debug start event")
...
for forbidden in ("Directory.Move(", "Directory.Delete(", "File.Move(", "File.Delete("):
    if forbidden in ollama_platform_417:
        failures.append(f"4.1.7 Ollama discovery safety regressed: {forbidden} found in platform discovery")
...
if "\x08" in setup_service_417:
    failures.append("4.1.7 model-size parser contains a literal backspace control character")
if r"(?:\b|$)" not in setup_service_417:
    failures.append("4.1.7 model-size parser lost its word-boundary suffix")
...
try:
    ...
    if set(localization_en_417) != set(localization_de_417):
        failures.append("4.1.7 localization parity: en-US/de-DE key sets differ")
    if len(localization_en_417) != 2256 or len(localization_de_417) != 2256:
        failures.append(f"4.1.7 localization parity: expected 2256 keys each, got {len(localization_en_417)}/{len(localization_de_417)}")
except Exception as exc:
    failures.append(f"4.1.7 localization parity files are unreadable: {exc}")
...
for token in ("@rendermode InteractiveServer", "Components/Pages/Error.razor", "must inherit MenuIsland's InteractiveServer circuit"):
    if token not in render_policy:
        failures.append(f"4.1.7 render-mode policy regressed: missing {token!r}")

if failures:
    print("LocalGPT 4.1.7 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.1.7 release audit passed: bounded browser chunk recovery and no-monolithic-fallback policy are guarded while ConfigurationRoot qualification, adaptive low-memory documentation, provider management, Matrix controls, renderer affinity and earlier macOS protections remain intact.")
