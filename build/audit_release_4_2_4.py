...
for relative in (
    ...
    version(relative, "4.2.4")

for relative, token in (
    ("docs/docfx.json", '"localgptVersion": "4.2.4"'),
    ("docs/pdf/toc.yml", "LocalGPT-4.2.4.pdf"),
    ("docs/pdf-cover.html", "LocalGPT 4.2.4 Documentation"),
    ("docs/index.md", "**Version 4.2.4**"),
    ("src/LocalGPT/Components/App.razor", "localgpt-chat-ui.js?v=4.2.4"),
    ("src/LocalGPT/Services/CanIRunHardwareRecommendationService.cs", "LocalGPT/4.2.4"),
    ("src/LocalGPT/Services/InitialSetupAssistantService.cs", "LocalGPT/4.2.4"),
    ("docs/reference/ai-provider-installation.md", "LocalGPT/4.2.4"),
...
# 4.2.4 replayable ASCII chat and contextual fun capability contract.
...
for token in (
    ...
    if token not in chat_424:
        failures.append(f"4.2.4 Chat ASCII presentation UI missing {token!r}")
for token in (
    ...
    if token not in chat_code_424 and token not in chat_424:
        failures.append(f"4.2.4 canonical chat mirror/state wiring missing {token!r}")
for token in (
    'LOCALGPT // ASCII CHAT',
    'BuildAsciiConversationTranscript',
    'ExtractAsciiSequenceFrames',
    '[THINK]',
    '[CALL ',
    '[RESULT ',
    '[THINK / FUNCTION TRACE]',
    '[SAY]',
    'BuildCouncilParticipantAsciiBody',
    'Shared /chat history · replayable when switching views',
):
    if token not in chat_console_424 and token not in chat_console_code_424:
        failures.append(f"4.2.4 ASCII transcript rendering missing {token!r}")
for token in (
    ...
    if token not in chat_console_js_424:
        failures.append(f"4.2.4 bounded ASCII sequence playback missing {token!r}")
for token in (
    ...
    if token not in ascii_state_424:
        failures.append(f"4.2.4 model ASCII capability guidance missing {token!r}")
if 'public interface IChatAsciiExperienceState' not in ascii_interface_424:
    failures.append("4.2.4 ASCII presentation state interface is missing")
if 'AddScoped<IChatAsciiExperienceState, ChatAsciiExperienceState>()' not in registration_424:
    failures.append("4.2.4 ASCII presentation state is not circuit-scoped in DI")
if '_councilText.AddOptionalSystemMessage(systemMessages, _asciiExperience.BuildModelGuidance(), _logger);' not in composite_424:
    failures.append("4.2.4 individual provider prompts do not receive ASCII presentation guidance")
if 'IChatAsciiExperienceState asciiExperience' not in factory_424:
    failures.append("4.2.4 chat client factory does not supply ASCII presentation state")
if 'IChatAsciiExperienceState asciiExperience' not in council_424 or 'asciiExperience.BuildModelGuidance()' not in council_participant_424:
    failures.append("4.2.4 Council participants do not receive the same ASCII presentation guidance")
if 'requestAnimationFrame(() => renderSequenceFrame' in chat_console_js_424:
    failures.append("4.2.4 ASCII sequence playback regressed to a per-display-frame animation loop")
    ...
    for raw_key in ("ASCII fun mode (contextual)", "Show ASCII terminal", "Hide ASCII terminal"):
        ...
        for prefix in ("Phrase.", "Text."):
            if f'"{prefix}{encoded_key}"' not in locale_text:
                failures.append(f"4.2.4 ASCII localization missing {prefix}{encoded_key} in {locale}")
...
for token in (
    ...
    if token not in ascii_dx_424:
        failures.append(f"4.2.4 ASCII DXFunction capability discovery missing {token!r}")
if blueprints_424.count('"localgpt.ascii.surface.get"') < 2 or '"localgpt.ascii.surface.get"' not in game_director_blueprint_424:
    failures.append("4.2.4 shipped ASCII/game Council blueprints do not seed the surface capability")
if 'registry.GetFunctions().Select(CreateDxEntry)' not in dx_catalog_424 or 'IsSystemSeed = !string.Equals(function.Source, "UserDxFunction"' not in dx_catalog_424:
    failures.append("4.2.4 ASCII DXFunction is not covered by the normal catalog/system-seed synchronization path")
for function_name in (
    ...
    if function_name not in game_dx_424:
        failures.append(f"4.2.4 existing ASCII game DXFunction regressed: {function_name}")
if '.Take(12)' not in chat_console_code_424 or 'Math.Clamp(frames, 0, 12)' not in chat_console_code_424:
    failures.append("4.2.4 server ASCII sequence projection is not bounded to 12 frames")
...
for source_path in (root / "src" / "LocalGPT").rglob("*.cs"):
    ...
    for method_name in ("Contains", "StartsWith", "EndsWith"):
        ...
        if re.search(pattern, source_text):
            failures.append(f"4.2.4 compiler-risk overload remains in {source_path.relative_to(root)}: char + StringComparison {method_name}")
...
for text, label in ((provider_models_424, "provider runtime authority"), (ollama_process_424, "Ollama health authority")):
    if "StartsWith('[', StringComparison.Ordinal)" in text:
        failures.append(f"4.2.4 {label} regressed to the invalid char/StringComparison StartsWith overload")
    if 'Contains(":", StringComparison.Ordinal)' not in text or 'StartsWith("[", StringComparison.Ordinal)' not in text:
        failures.append(f"4.2.4 {label} does not retain the compiler-safe IPv6 string overloads")

if failures:
    print("LocalGPT 4.2.4 release audit FAILED:")
    print("\n".join(f" - {failure}" for failure in failures))
    sys.exit(1)
print("LocalGPT 4.2.4 release audit passed: replayable ASCII chat includes canonical messages plus server-owned Council participant lanes, the ASCII surface capability is exposed through the normal system-seeded DXFunction catalog and preferred by shipped ASCII Council teams, contextual ASCII art/sequences remain opt-in and bounded to 12 frames, browser/gamepad churn is bounded, Ollama health recovery verifies the local API, Council resource identity is protocol-independent and port-aware, TLS catalog failure preserves certificate validation with offline fallback, and prior provider canonicalization protections remain intact.")
