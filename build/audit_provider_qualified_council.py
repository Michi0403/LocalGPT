...
def main() -> int:
    ...
    contains("bounded-number editor uses compact DevExpress numeric input", bounded_number, "<DxSpinEdit")
    contains("bounded-number editor keeps readable min/current/max hints", bounded_number, "bounded-number-hint")
    excludes("bounded-number editor no longer embeds a chart-like range selector", bounded_number, "DxRangeSelector")
    ...
    excludes("provider runtime no longer treats one Ollama /v1 facade as an independent runtime", runtime, "Native Ollama and its OpenAI-compatible /v1 surface are deliberately separate")
    contains("provider runtime canonicalizes confirmed same-runtime Ollama /v1 aliases", runtime, "IsOllamaOpenAiCompatibilityFacade")
