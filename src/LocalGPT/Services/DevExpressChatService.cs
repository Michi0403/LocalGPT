...
namespace LocalGPT.Services
{
    ...
    [
        ...
        new(
            ...
            "take optional; DI invocation uses the database-backed MaxFiles policy when omitted or non-positive.",
        ...
        new(
            ...
            "workspaceName from council.artifact_workspaces; take optional and database-policy-backed through DI invocation.",
            "Read-only. Only database-provisioned source/docs text extensions are listed; binaries and build outputs are excluded."),
        new(
            "council.artifact_workspace_file.read",
            "POST",
            "/api/dxai/functions/council.artifact_workspace_file.read/invoke",
            "Read one generated source/docs file by relative path so the AI and user can iterate before zipping.",
            "JSON: workspaceName, relativePath.",
            "Read-only. Paths stay inside the selected generated workspace and use the database-provisioned text-extension policy."),
        new(
            "council.artifact_workspace_file.write",
            "POST",
            "/api/dxai/functions/council.artifact_workspace_file.write/invoke",
            "Write one reviewed plain text/source file, including PowerShell .ps1, when direct source output is preferable to CodeDOM or CodeDOM is unavailable.",
            "JSON: workspaceName, relativePath, content.",
            "Writes only supported text-like files inside the selected generated workspace and never executes them. Fresh human approval is required.",
            false,
            true),
        new(
