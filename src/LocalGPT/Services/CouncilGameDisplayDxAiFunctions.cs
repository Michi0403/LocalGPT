...
public sealed class GetCouncilGameDisplayFunction(ICouncilGameSessionService games, CouncilGameDxParameterReader parameters, ILogger<GetCouncilGameDisplayFunction> logger) : IDxAiFunctionHandler
{
    ...
    public DxaichatFunctionInfo Descriptor { get; } = new(
        ...
        "sessionId optional; when omitted LocalGPT resolves the active conversation game. x, y, width and height optional. Zero width/height means through the display edge.",
    ...
    public async Task<DxAiFunctionInvocationResult> InvokeAsync(DxAiFunctionInvocationRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (sessionId == Guid.Empty)
            {
                ...
                if (active is null)
                    return new() { Succeeded = false, Status = "NotFound", Error = "No active game exists for this conversation." };
                sessionId = active.Id;
            }
