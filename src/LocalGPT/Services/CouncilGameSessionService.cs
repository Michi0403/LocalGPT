...
public sealed partial class CouncilGameSessionService : ICouncilGameSessionService, IDisposable
    ...
    /// <summary>Retrieves the newest running game of one specific family owned by one Council run.</summary>
    /// <param name="councilRunId">Identifier of the Council run that owns the game.</param>
    ...
    public Task<CouncilGameSessionSnapshot?> GetActiveForCouncilRunAsync(Guid councilRunId, string gameKey, CancellationToken cancellationToken = default)
    {
        try
        {
            ThrowIfDisposed();
            ...
            var normalizedGameKey = NormalizeGameKey(gameKey);
            var session = sessions.Values
                .Where(item => item.Status == "Running"
                    && item.CouncilRunId == councilRunId
                    && string.Equals(item.GameKey, normalizedGameKey, StringComparison.OrdinalIgnoreCase))
                .OrderByDescending(item => item.UpdatedAtUtc)
                .FirstOrDefault();
            return Task.FromResult(session is null ? null : ToSnapshot(session));
        }
        ...
        {
            logger.LogInformation(exception, "Reading the active Council-run game session was cancelled for {CouncilRunId} and game family {GameKey}.", councilRunId, gameKey);
            throw;
        }
        catch (Exception exception)
        {
            logger.LogError(exception, "Reading the active Council-run game session failed for {CouncilRunId} and game family {GameKey}.", councilRunId, gameKey);
            throw;
        }
    }
