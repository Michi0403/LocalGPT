    ...
    ILocalGptApplicationPathService applicationPaths) : IInitialDataCatalog
{
    ...
    [
        ...
        new("project.evidence::SourceCode::DotNetProject::DotNet-MSBuild", "(?i)(?:^|/)(?:[^/\\r\\n]+\\.slnx?|[^/\\r\\n]+\\.(?:csproj|vbproj|fsproj))$", "i,c"),
        new("project.evidence::SourceCode::JavaMavenProject::Java-Maven", "(?i)(?:^|/)pom\\.xml$", "i,c"),
        new("project.evidence::SourceCode::JavaGradleProject::Java-Gradle", "(?i)(?:^|/)build\\.gradle(?:\\.kts)?$", "i,c"),
        new("project.evidence::SourceCode::NodeProject::Node-PackageManager", "(?i)(?:^|/)(?:package\\.json|pnpm-lock\\.yaml|yarn\\.lock)$", "i,c"),
        new("project.evidence::SourceCode::RustProject::Rust-Cargo", "(?i)(?:^|/)Cargo\\.toml$", "i,c"),
        new("project.evidence::SourceCode::GoProject::Go", "(?i)(?:^|/)go\\.mod$", "i,c"),
        new("project.evidence::SourceCode::PythonProject::Python", "(?i)(?:^|/)(?:pyproject\\.toml|requirements\\.txt|Pipfile|poetry\\.lock)$", "i,c"),
        new("project.evidence::GameProject::MinecraftProject::Minecraft", "(?i)(?:^|/)(?:fabric\\.mod\\.json|mods\\.toml|neoforge\\.mods\\.toml|paper-plugin\\.yml|plugin\\.yml|pack\\.mcmeta)$", "i,c"),
        new("project.evidence::Documentation::DocumentSet::Knowledge", "(?i)\\.(?:md|mdx|txt|rst|adoc|pdf|docx?|rtf|odt)$", "i,c"),
        new("project.evidence::StructuredData::Dataset::DataAnalysis", "(?i)\\.(?:csv|tsv|json|xml|ya?ml|xlsx?|ods|parquet|sqlite|db)$", "i,c"),
        new("project.evidence::ImageMedia::ImageSet::Vision", "(?i)\\.(?:png|jpe?g|webp|gif|svg)$", "i,c"),
        new("project.evidence::AudioVideoMedia::MediaSet::MediaAnalysis", "(?i)\\.(?:mp4|mov|mkv|avi|mp3|wav|srt)$", "i,c"),
        new("project.evidence::Archive::ArchiveSet::ArchiveReview", "(?i)\\.(?:zip|7z|rar|tar|gz|xz|zst)$", "i,c"),
        new("project.evidence::EngineeringDesign::CadProject::DesignEngineering", "(?i)\\.(?:dwg|dxf|step|stl|kicad_(?:sch|pcb|pro|sym|mod|wks))$", "i,c"),
        new("project.evidence::EmbeddedSource::EmbeddedProject::EmbeddedToolchain", "(?i)(?:^|/)(?:platformio\\.ini|CMakeLists\\.txt|idf_component\\.yml|[^/]+\\.ino)$", "i,c"),
...
"""),
        ...
        new("CodeGenerationChangeReviewPolicy", "en",
            "Before LocalGPT writes generated source, scripts, addons, solutions, DLL projects, or executable projects, create a database-backed change-review snapshot through codegen.review.create. Supply concrete files, CodeDOM types, or an output target whenever the user requested a concrete artifact; do not print transport/tool JSON as the final user answer. The review must summarize the current project state, council decision, proposed files and CodeDOM types, output targets, safety boundary, and exact review hash. Immediately route that exact review through codegen.review.execute so the normal Human Collaboration approval card exists; do not stop at an orphaned review or ask the human to invoke the function manually. HumanApprovalPending means the consequential write/build is correctly queued and must wait for the local decision, while independent work may continue. Approved deferred generation resumes from the Human Collaboration Inbox or on a council heartbeat. Generation approval is one-use and hash-bound; a requested .NET build is part of the same exact reviewed generation action and must still receive the registry's current approval before execution. Generated programs, scripts, DLLs, and addons are never executed or loaded automatically."),
    ...
    /// <summary>Gets previous repository-owned prompt defaults that may be upgraded only while the persisted row still exactly matches the old shipped text.</summary>
    /// <value>Prior defaults used to preserve user-edited prompt rows while allowing deterministic seed corrections.</value>
    public IReadOnlyList<PromptConfigDto> LegacyPromptDefaults { get; } =
    [
        new("CodeGenerationChangeReviewPolicy", "en",
