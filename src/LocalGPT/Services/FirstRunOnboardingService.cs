    ...
    ILogger<FirstRunOnboardingService> logger) : IFirstRunOnboardingService
{
    ...
    {
        return [
        QuickStart("benchmark", "Calibrate installed models first", "Recommended after installation: Task Curators prepare one checkable four-task pack, every selected Council member executes that assigned role task pack, LocalGPT measures every benchmark-capable provider-qualified member across five bounded profile points using the full maintained task suite, and stores measured Low, Normal, High, Expert and Max hardware profiles before later work.", "adaptive-model-benchmark", "Initial Calibration Pool", "benchmark-council-start"),
