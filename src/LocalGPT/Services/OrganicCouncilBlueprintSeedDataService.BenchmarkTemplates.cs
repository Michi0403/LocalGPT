...
namespace LocalGPT.Services
{
        ...
        return new()
    {
        ...
        Purpose = "First-run benchmark structure for every selected provider-qualified Council member. The workflow freezes one exact target set, Task Curators prepare one consolidated four-section benchmark suite, and LocalGPT executes that same suite as one deterministic measurement phase across every benchmark-capable Benchmark Subject. Four bounded token profile points are measurements inside that single phase, never four model packs or four target subsets. Independent roles only review the complete all-subject measurement matrix after execution and then synthesize measured Low, Middle, High and Expert hardware-spooler profiles.",
...
This is ONE deterministic LocalGPT measurement phase using the immediately preceding consolidated Task Curator suite as the authoritative assignment. Every distinct benchmark-capable provider-qualified Benchmark Subject in the frozen Council target set is forced through the same suite; none may be replaced by a representative or silently omitted. The four suite sections are bundled into one provider turn at each bounded token profile point. The four profile points are parameter measurements of the same subject and suite, not four packs, slides or model groups. Independent physical/provider hosts may run in parallel while models sharing one host remain sequential to avoid VRAM contention. Generic AI-capability refusal receives one bounded same-role retry. Failed/unsupported members remain explicit evidence.
...
""", "AllMembersParallel", canUseOrganicFunctions: true, producesFinalAnswer: true, enableRolePeerReview: true, summarizeRoleResults: true, allowedAutomaticFunctions: ["localgpt.hardware.performance.presets.list", "localgpt.hardware.performance.presets.get"])
        ...
        [
            ...
            "The four curator sections are executed together in one bounded provider turn at each of four profile points. Those profile points are repeated parameter measurements inside one visible measurement phase, not four benchmark packs; all selected subjects remain in the same authoritative coverage set.",
