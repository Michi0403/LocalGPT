...
Your assigned role task is coverage accounting only. Audit the deterministic calibration evidence. The deterministic summary contains a section named `Machine-derived coverage invariant` with attempted count, successful count, unresolved count, arithmetic check and the exact authoritative unresolved provider-qualified identity list. Copy those machine-derived counts and identities exactly. Before answering verify `attempted - successful = unresolved`. Do not infer a smaller unresolved list from examples, one host, quality-review prose or your own memory. If any Council/model prose conflicts with the machine-derived invariant, explicitly treat that prose as incorrect and preserve the deterministic set. A failed measurement is evidence, not permission to extrapolate. State PASS only when the authoritative unresolved set is empty; otherwise state PARTIAL and list every authoritative unresolved identity. Do not redesign tasks or profiles. Coordinate peer usefulness/votes into one role result.
...
Your assigned role task is final profile synthesis only. Produce the final visible calibration handoff from the accumulated benchmark evidence. The deterministic machine-derived coverage invariant and exact unresolved provider-qualified identity list are authoritative; if prior reviewer prose disagrees, correct the prose rather than the benchmark evidence. The intended five calibration tiers are Low, Normal, High, Expert and Max; name the tiers LocalGPT actually stored and explicitly report any tier that was omitted because no subject completed that exact measurement point. Explain that stored tiers contain only successful measured provider-qualified routes, that failed/unsupported selected members remain explicit coverage gaps, and that applying a hardware profile never changes Council membership. Recommend Low as the conservative first-run baseline, Normal for ordinary use, High for higher-value work, Expert for demanding work, and Max only when its measured route exists and the workload justifies it. Do not claim that a missing route was benchmarked. Consolidate the two synthesizer members into one final role result.
""", "AllMembersParallel", canUseOrganicFunctions: true, producesFinalAnswer: true, enableRolePeerReview: true, summarizeRoleResults: true, allowedAutomaticFunctions: ["localgpt.hardware.performance.presets.list", "localgpt.hardware.performance.presets.get"])
        ...
        [
            ...
            "Coverage counts and unresolved provider-qualified identities are mechanically derived from the benchmark report. Council/model prose may interpret them but must never replace, shrink or expand the authoritative machine-derived identity set."
        ]
    };
    }
