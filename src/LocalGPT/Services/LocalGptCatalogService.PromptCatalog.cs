...
namespace LocalGPT.Services
{
    ...
    try
    {
                ...
                [
                    ...
                    new("Council starter: AI-guided initial setup", "Guide hardware, provider, model and benchmark setup", "Start the maintained AI-guided Initial Setup Team. Begin with initial.setup.status. Use human.collaboration.request for every user choice, keep CanIRun.ai optional and attributed, detect existing providers/models before proposing installation, use only confirmation-gated setup DXFunctions for changes, and finish by creating or refreshing a hardware-curated benchmark team whose curator/reviewer pool prefers the strongest installed models.", "initial-setup-council-start", ["initial-setup-assistant"], true),
                    new("Council starter: calibrate installed models", "Run the recommended first-install model calibration", "Start the maintained Initial Hardware Calibration Benchmark Council. Keep the selected provider-qualified membership authoritative: prepare one role-owned checkable task pack, make every selected Council member execute that assigned Benchmark Subject task pack, deterministically benchmark every benchmark-capable selected member with the full maintained four-task suite at five bounded profile points, preserve failures as coverage evidence, and store measured Low, Normal, High, Expert and Max hardware profiles before synthesis. Do not sample representatives and do not overwrite unrelated approved presets.", "benchmark-council-start", ["adaptive-model-benchmark"], true),
