    ...
    ILogger<GetProjectMaintenanceFunction> logger) : IDxAiFunctionHandler
{
    ...
    public async Task<DxAiFunctionInvocationResult> InvokeAsync(DxAiFunctionInvocationRequest request, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
            Project = new { details.Project.Id, details.Project.Name, details.Project.ProjectType, details.Project.CurrentVersion, details.Project.RootPath, details.Project.SolutionPath, details.Project.SolutionSearchPattern, details.Project.FileIncludePattern, details.Project.FileExcludePattern },
            Workspace = workspace,
            ...
            Compilers = compilers.Where(item => item.IsEnabled).Select(item => new { item.Id, item.Name, item.Language, item.ToolchainKind, item.DetectedPlatform, item.ExecutablePath, item.CompilerHomePath, item.Version, item.Architecture, item.DiscoverySource, item.KnowledgeProfileKey, item.KnowledgeEntryId, item.VersionKnowledgeEntryId, item.EnvironmentVariables, item.LastValidationSucceeded, item.IsDefaultForLanguage }),
            Versions = details.Versions.Select(item => new { item.Id, item.Version, item.Notes, item.PathSnapshot, item.IsCurrent, item.CreatedAtUtc }),
            ...
            Artifacts = details.Artifacts.Select(item => new { item.Id, item.RevisionId, item.RequirementId, item.ArtifactKind, item.Name, Value = item.IsSensitive ? "[sensitive]" : item.Value, item.DataType, item.Flags, item.Description, item.CouncilReviewStatus, item.IsUserApproved, item.UpdatedAtUtc }),
