...
const proofPath = path.join(root, 'WORKSPACE-CONTENT-PROOF-v0.1.8.json');
const proof = JSON.parse(fs.readFileSync(proofPath, 'utf8'));
const ignoredParts = new Set(['bin', 'obj', 'node_modules', '.git', '.vs']);
const ignoredNames = new Set([
  'WORKSPACE-BASELINE-MANIFEST-v0.1.7.json',
  'WORKSPACE-CONTENT-PROOF-v0.1.8.json',
  'WORKSPACE-CONTENT-PROOF-v0.1.8.md'
]);
...
console.log(`LocalGPT workspace preservation proof passed: ${proof.baselineFileCount} baseline files retained, ${proof.addedFileCount} files added, zero baseline paths lost.`);
