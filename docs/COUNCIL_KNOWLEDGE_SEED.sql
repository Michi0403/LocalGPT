...
(
  ...
  'A local AI host .NET/DevExpress generation must not be only a few web pages. It should include an ASP.NET Core control-plane API with representative provider-compatible route stubs for version, tags/list models, running models, show, pull, push, create, copy, delete, generate, chat, and embed. It should include DevExpress pages for API console, model catalog, model download planning, chat, running models, logs, and settings. It must clearly say native GGML/GPU inference, model loading, and real binary downloads are not implemented unless an approved backend exists.',
