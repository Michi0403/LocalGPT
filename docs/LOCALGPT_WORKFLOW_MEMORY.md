...
- Council/product-workflow work has a mandatory WebView2 visible-UI preflight unless the user explicitly labels the task documentation-only. Preflight passes only when the real WinUI WebView2 window is visible, the target page is in its post-prerender interactive state, loading/startup overlays are gone, the route remains stable after observation, and the exact controls needed for the next action are visible and enabled. For LocalGPT, prerendered markup, transient route changes, disabled controls, stale selectors, and typed-but-not-sent text are not valid workflow states.
- WebView2 control must be human-paced. Perform one visible UI action, wait for the frontend rendering/control state to settle, inspect the result, then continue. Do not fire many navigation, typing, click, or keyboard commands in a few milliseconds; that is not human behavior and cannot prove a LocalGPT product workflow.
...
- `diagSession` and diagnostic query-string mutation are not valid controls for
  DXAiChat/AI Council product workflow state. They may briefly change the
  Blazor route and then settle back, or leave the page in a transient
  prerender/interactive state. Agents must not rely on URL mutation to create a
  fresh chat, select a Council session, or prove a result. Use the visible UI
  controls instead: navigate normally to Chat, press Start New Chat when a fresh
  context is needed, select models through the picker, set the artifact toggle,
  type into the real textbox, send with the visible button, and wait when the UI
  is not stable. If this workflow needs automation support, build an explicit
  user-facing or test-only in-app feature rather than overloading diagnostic
  query parameters.
