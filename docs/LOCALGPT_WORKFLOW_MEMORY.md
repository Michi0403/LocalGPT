...
- Token budget lesson from DXAiChat testing: 4K/8K context is only a smoke-test budget. Serious local code generation should start around 32K context, use 64K+ when generating larger files or whole solutions, and keep controls open up to 256K for Ollama/model combinations that support it. If a reasoning model emits only thinking, LocalGPT should recover with a short final-answer-only continuation instead of leaving the chat with a spinner or empty answer.
