...
- `diagSession` and diagnostic query-string mutation are not valid controls for DXAiChat/AI Council product workflow state. They may briefly change the Blazor route and then settle back, or leave the page in a transient prerender/interactive state. Agents must not rely on URL mutation to create a fresh chat, select a Council session, or prove a result. Use the visible UI controls instead: navigate normally to Chat, press Start New Chat when a fresh context is needed, select models through the picker, set the artifact toggle, type into the real textbox, send with the visible button, and wait when the UI is not stable. If this workflow needs automation support, build an explicit user-facing or test-only in-app feature rather than overloading diagnostic query parameters.
