...
LocalGPT 4.1.7 extends the selected local-provider setup card with a DevExpress runtime-management workbench. The workbench keeps provider-native lifecycle behavior explicit: **Unload** releases memory without deleting the download, while **Remove permanently** is available only where the provider exposes a documented deletion operation. The grid shows provider-reported model size, architecture, parameter count, loaded state, and maximum context length when the provider reports it. A low-free-space warning is shown for LocalGPT-managed Ollama storage so large model collections can be cleaned before the filesystem is exhausted.
