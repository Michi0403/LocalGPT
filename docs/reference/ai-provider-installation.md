...
LocalGPT 4.3.0 extends the selected local-provider setup card with a DevExpress runtime-management workbench. The workbench keeps provider-native lifecycle behavior explicit: **Unload** releases memory without deleting the download, while **Remove permanently** remains a separately confirmed destructive action. Ollama storage evidence is grouped by actual drive/mount and store path: each volume reports free/total capacity and physical blob bytes, while each model row reports a logical size that may overlap when blobs are shared. Direct symbolic-link store targets are resolved before volume attribution so a linked default store is not presented as a second physical copy.
