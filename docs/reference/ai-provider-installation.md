...
LocalGPT 4.2.1 extends the selected local-provider setup card with a DevExpress runtime-management workbench. The workbench keeps provider-native lifecycle behavior explicit: **Unload** releases memory without deleting the download, while **Remove permanently** is available only where the provider exposes a documented deletion operation. The grid shows provider-reported model size, architecture, parameter count, loaded state, and maximum context length when the provider reports it. A low-free-space warning is shown for LocalGPT-managed Ollama storage so large model collections can be cleaned before the filesystem is exhausted.

For **Ollama**, LocalGPT can persist a model directory for LocalGPT-started CLI instances, including an external macOS volume selected through the host-path explorer. The saved directory is supplied through `OLLAMA_MODELS`; LocalGPT never silently moves existing blobs or manifests. Stop Ollama and deliberately relocate any existing store before deleting the old copy. The same workbench exposes `OLLAMA_HOST`, `OLLAMA_CONTEXT_LENGTH`, `OLLAMA_KEEP_ALIVE`, `OLLAMA_MAX_LOADED_MODELS`, `OLLAMA_NUM_PARALLEL`, `OLLAMA_MAX_QUEUE`, and local-only `OLLAMA_NO_CLOUD` behavior. Bind choices are intentionally limited to loopback (`127.0.0.1`) or all IPv4 interfaces (`0.0.0.0`) so arbitrary remote hosts remain separate provider profiles. Permanent model removal uses Ollama's documented delete API and requires fresh destructive-action acknowledgement.
