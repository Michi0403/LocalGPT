...
The preseeded `ascii-doom-council-adventure` team treats `CouncilGameSessionService` as the only game engine. Starting the team creates or recovers one deterministic corridor session in **Human + AI** (`Shared`) control mode, with background autoplay off. Human input remains available immediately; selecting **Human**, **Human + AI**, or **AI hunter** in the Game tab changes runtime ownership without changing the Council team. The Council observes and explains the authoritative state; it does not run a second model-invented simulation.

The starter team assigns the resettable `games.ascii.doom.campaign` runtime class. Its visible class fields define the starting level, automatic level advance, and a configurable level-profile set containing map width/height, room count, enemy density, deterministic difficulty rating, enemy health/damage multipliers, and starting health/ammunition. The supplied class contains ten progressively harder starter levels. Copy the runtime class, edit any of those values or the level count/order, and assign the copied class to a copied team role to create a different campaign without changing engine code. The game service resolves a copied team assignment before the system seed and clamps the configured values to safe deterministic bounds; it does not invent a hidden replacement difficulty curve.

The active campaign class drives the connected map, persistent hostile actors, opening line-of-sight contact, ammunition/health, extraction coordinates, collision feedback, tactical radar, and automatic transition to the next configured level after a cleared extraction when `autoAdvanceLevels` is enabled. In **Human** mode AI-origin movement is rejected. In **Human + AI**, both sides may submit explicit bounded controls but no background autoplay loop runs. **AI hunter** is the only autonomous mode. The hunter reads the authoritative map, turns toward visible hostiles, shoots through validated line of sight, pathfinds around walls and occupied hostile cells, and routes to extraction after combat. Shooting, enemy HP, enemy pursuit/contact attacks, death, level progression, and extraction are deterministic service-owned transitions rather than model narration.
