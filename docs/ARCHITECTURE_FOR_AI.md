...
- users can start a dedicated implementation-request council chat. This enables sandbox implementation artifacts for .NET/Blazor/ASP.NET Core style feature ideas and exposes them through safe `/__artifacts/council/{fileName}` download links. For Blazor frontend requests, LocalGPT should generate a real `.razor` component/page artifact plus compileable `.cs` support code; a C# class that only builds strings is not enough unless the user asked for that shape. Requested features should be prototyped in a harmless sandbox artifact or temporary workspace and smoke-tested before integration into the real project structure. LocalGPT must never self-expand or integrate generated features without explicit user permission, and a user decision that denies or limits expansion must not be overruled.
