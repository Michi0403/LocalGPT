    ...
    ILogger<DatabaseInitializationService> logger) : IDatabaseInitializationService
{
    ...
    private async Task SeedCoreProjectsAsync(LocalGptMemoryDbContext db, CancellationToken token)
    {
        ...
        EnsureVersion(core, "2.2.10", repositoryRoot, "Chat ASCII-console close-action release with fullscreen-safe removal in every display scale mode.");
        EnsureRevision(core, "main", "seed-v2.2.10", repositoryRoot,
