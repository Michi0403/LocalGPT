        ...
        public async Task<string> BuildBootstrapPromptAsync(CancellationToken cancellationToken = default)
        {
            try
            {
                var builder = new StringBuilder()
                    ...
                    .AppendLine("Structured work rule: before calling any function, identify the active database project/revision, restate the relevant approved requirements, list the minimum evidence still missing, and select only functions whose declared capability directly advances that plan. Do not browse unrelated tables or call loosely related tools merely because they are available. Treat project-linked artifact names and requirement links as discovery metadata; sensitive values and exact table contents still require their normal authorization path.")
                    .AppendLine("Database self-maintenance rule: before any substantial AI Chat or Council task, inspect the relevant project, approved chat memory, knowledge, regex, diagnostic-log, repository-document, build/debug and DXFunction directory evidence. Fill deterministic seed gaps and safe knowledge-index gaps without asking for permission; do not silently overwrite user-maintained rows. Ask the user for missing volatile facts such as current compiler/framework versions, scientific constants, source revisions, device capabilities or exact requirements instead of guessing.")
                    .AppendLine("The LocalGPT Core and Humanitarian Collaboration Workspace projects are permanent database anchors. Use compact links between projects, topics, requirements, regexes, knowledge and artifacts rather than copying whole histories into each prompt. The single AI Chat may use the same Council, project, memory, regex, diagnostics and organic 1-Wire functions that are explicitly advertised by the DI-backed DXFunction directory.")
                    .AppendLine("At every Council round and heartbeat, leaders, experts and members must re-check their configured CPU/GPU/accelerator road, per-model token range, current DXFunction directory, approved skill evidence, connected 1-Wire organs and unresolved human questions. A new or previously unknown member must introduce itself and provide only an untrusted self-assessment until the user approves it.")
