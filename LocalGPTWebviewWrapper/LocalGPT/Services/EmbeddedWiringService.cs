...
""".Trim();
    ...
    private string NormalizeProtocol(string? value) => string.IsNullOrWhiteSpace(value) ? EmbeddedProtocolKeys.Custom : value.Trim().ToLowerInvariant();
    private string NormalizeId(string? value) => new string((value ?? string.Empty).Select(ch => char.IsLetterOrDigit(ch) ? char.ToLowerInvariant(ch) : '-').ToArray());
    private bool IsGround(EmbeddedWiringNode node) => string.Equals(node.ElectricalRole, "Ground", StringComparison.OrdinalIgnoreCase) || string.Equals(node.PinKey, "GND", StringComparison.OrdinalIgnoreCase);
    private bool IsPower(EmbeddedWiringNode node) => string.Equals(node.ElectricalRole, "Power", StringComparison.OrdinalIgnoreCase);
    private bool IsOutput(EmbeddedWiringNode node) => (node.Direction ?? string.Empty).Contains("Output", StringComparison.OrdinalIgnoreCase) || (node.Direction ?? string.Empty).Contains("Drive", StringComparison.OrdinalIgnoreCase);
