        ...
        ILogger<LocalGptCatalogService> logger)
    {
        ...
        public List<PromptSuggestion> GetSuggestion()
        {
            ...
            [
                ...
                new("Council starter: project work", "Start the general Organic Project Team", "Start a fresh Organic Project Team Council run. Ask me for the exact project goal, current state, repository or workspace, compiler evidence and approval boundaries, then execute the maintained preparation, architecture, implementation, verification and consensus workflow.", "general-project-council-start", ["general"], true),
                ...
                new("Minecraft target choice", "Pick Fabric, NeoForge, Paper, or datapack", "Act as a LocalGPT AI Council member. Compare Fabric mod, NeoForge mod, Paper plugin, vanilla datapack, and future Bedrock add-on for my request. Recommend one target, explain setup, and create a short poll if a decision or missing tool blocks progress.", "minecraft-target-choice", ["minecraft-development"], true),
                new("Minecraft mod plan", "Plan a buildable Java mod or plugin", "Act as a senior Minecraft Java engineer. Create a buildable Fabric, NeoForge, or Paper plan with exact classes, registry or command steps, assets/data files, Gradle commands, and risks. If LocalGPT is missing a needed feature, include a 'Missing feature report' section.", "minecraft-mod-plan", ["minecraft-development"], true),
                new("Minecraft datapack", "Generate vanilla datapack files", "Generate a vanilla Minecraft Java datapack. Include pack.mcmeta, load/tick function tags, namespace functions, scoreboard/storage design, validation steps, install commands, and performance notes. If AI Council downloadable artifacts are enabled, create a download-ready datapack zip.", "minecraft-datapack", ["minecraft-development"], true),
                new("Datapack debug", "Find why /function cannot see files", "Debug a Minecraft Java datapack whose function is not visible in /function. Check zip root layout, pack.mcmeta, pack_format, singular/plural function folders for the target version, load/tick tags, namespace/path casing, .mcfunction.txt mistakes, storage syntax, and provide exact file tree fixes.", "minecraft-datapack-debug", ["minecraft-development"], true),
                new("Living Cities datapack", "Generate a phased Living Cities datapack", "Use the Living Cities 0.1 technical plan as the target. Produce a buildable, download-ready datapack zip plus optional Java follow-up steps, file paths, commands, scoreboard/storage design, and performance notes for 1000+ citizens.", "living-cities-datapack", ["minecraft-development"], true),
                new("Missing features", "Write gaps to report file", "Review LocalGPT as a Minecraft mod builder. List missing features, blocked workflows, and required backend/frontend capabilities under a 'Missing feature report' heading.", "minecraft-missing-features", ["minecraft-development"], true),
