    ...
    ILogger<OneWireRuntimeSecurityService> logger) : IOneWireRuntimeSecurityService
{
    ...
    private static byte[] BuildSignatureBytes(OneWireEnvelope envelope) => JsonSerializer.SerializeToUtf8Bytes(new
    {
    ...
    private static byte[] BuildAssociatedData(OneWireEnvelope envelope) => Encoding.UTF8.GetBytes(
    ...
    private static bool IsSecurityBootstrap(OneWireMessageType type) => type is
