    ...
    ILogger<EmbeddedFirmwarePlanningService> logger) : IEmbeddedFirmwarePlanningService
{
    ...
    private string BuildArduinoSketch(EmbeddedFirmwarePlan plan)
    {
        ...
        return $$$"""// Generated by LocalGPT as a reviewable planning artifact.
// Verify the exact board, electrical limits, drivers and physical wiring before compile or flash.
#include <Arduino.h>
{{{includes}}}

constexpr unsigned long TELEMETRY_INTERVAL_MS = {{{plan.TelemetryIntervalMilliseconds}}}UL;
{{{declarations}}}
{{{oneWireDeclarations}}}
unsigned long lastTelemetry = 0;
unsigned long sequenceNumber = 0;

void publishReading(const char* sensorKey, const char* pinKey, int gpio, const char* metric, double value, const char* unit) {
  Serial.print("{\"schema\":\"localgpt.embedded.telemetry.v1\",\"deviceId\":\"{{{EscapeCpp(plan.DeviceName)}}}\",\"boardProfileKey\":\"{{{EscapeCpp(plan.BoardProfileKey)}}}\",\"transportKey\":\"{{{EscapeCpp(EmbeddedProtocolKeys.SerialJsonLines)}}}\",\"sequence\":");
  Serial.print(++sequenceNumber);
  Serial.print(",\"deviceTimestampMilliseconds\":"); Serial.print(millis());
  Serial.print(",\"readings\":[{\"sensorKey\":\""); Serial.print(sensorKey);
  Serial.print("\",\"pinKey\":\""); Serial.print(pinKey);
  Serial.print("\",\"gpio\":"); Serial.print(gpio);
  Serial.print(",\"metric\":\""); Serial.print(metric);
  Serial.print("\",\"value\":"); Serial.print(value, 4);
  Serial.print(",\"unit\":\""); Serial.print(unit);
  Serial.println("\",\"quality\":\"raw\"}],\"metadataJson\":\"{}\"}");
}

void setup() {
  Serial.begin({{{plan.BaudRate}}});
{{{setupPins}}}
{{{oneWireSetup}}}
}
...
""";
    ...
    private string BuildPlatformIoConfiguration(EmbeddedFirmwarePlan plan, EmbeddedBoardProfile? profile)
    {
        ...
        var libraryDependencies = plan.PinAssignments.Any(item => item.ProtocolKey == EmbeddedProtocolKeys.PhysicalOneWire)
            ...
            : string.Empty;
        return $$$"""[platformio]
default_envs = localgpt-embedded

[env:localgpt-embedded]
platform = {{{platform}}}
board = {{{board}}}
framework = arduino
monitor_speed = {{{plan.BaudRate}}}
{{{libraryDependencies}}}
build_flags =
  -DLOCALGPT_DEVICE_NAME=\"{{{EscapeIni(plan.DeviceName)}}}\"
  -DLOCALGPT_BOARD_PROFILE=\"{{{EscapeIni(plan.BoardProfileKey)}}}\"
""";
    }
    ...
    private string BuildWiringMarkdown(EmbeddedFirmwarePlan plan, EmbeddedBoardProfile? profile, string additionalRequirements)
    {
        ...
        return $$$"""# {{{plan.DeviceName}}} wiring and LocalGPT telemetry plan

Status: **{{{plan.OverallStatus}}}**

Board: {{{plan.BoardFamily}}} / {{{plan.BoardName}}}  
Board profile: `{{{plan.BoardProfileKey}}}`  
Framework: {{{plan.Framework}}}  
Transport: `{{{plan.TelemetryTransport}}}` at {{{plan.BaudRate}}} baud
...
| Pin | GPIO | Function | Protocol | Sensor key | Review |
|---|---:|---|---|---|---|
{{{rows}}}
...
The embedded device packet is not itself a trusted LocalGPT command. A local gateway validates source identity, bounds, replay/timestamp policy and capability routing before creating a protected logical 1-Wire envelope. Physical 1-Wire, I²C, SPI, UART, CAN, RS-485 or custom sensor buses remain independent choices.

## Additional user requirements

{{{(string.IsNullOrWhiteSpace(additionalRequirements) ? "None supplied." : additionalRequirements.Trim())}}}

## Learning round

{{{plan.LearningRoundAdvice}}}
""";
    }
